...
// preload bundle must not pull in `@sankiwork/desktop/main` (it
...
function actionFailure(reason: string, details?: unknown): SankiWorkHostActionResult {
  return failure(reason, details);
}

function importFailure(reason: string): SankiWorkHostProjectImportResult {
  return failure(reason);
}

function replaceWorkingDirFailure(reason: string): SankiWorkHostProjectReplaceWorkingDirResult {
  return failure(reason);
}

function normalizeProjectReplaceWorkingDirResult(input: unknown): SankiWorkHostProjectReplaceWorkingDirResult {
...
function pickWorkingDirFailure(reason: string): SankiWorkHostPickWorkingDirResult {
  return failure(reason);
}

function normalizePickWorkingDirResult(input: unknown): SankiWorkHostPickWorkingDirResult {
...
function normalizeProjectImportResult(input: unknown): SankiWorkHostProjectImportResult {
...
const shell = {
  openExternal: async (url: string): Promise<SankiWorkHostActionResult> => {
    try {
  ...
  openPath: async (projectId: string): Promise<SankiWorkHostActionResult> => {
    try {
...
const browser = {
  clearData: async (options?: SankiWorkHostBrowserClearDataOptions): Promise<SankiWorkHostActionResult> => {
    try {
...
const updater = {
  ...
  subscribe: (listener: SankiWorkHostUpdaterStatusListener): (() => void) => {
    const handler = (_event: unknown, status: SankiWorkHostUpdaterStatusSnapshot): void => {
      listener(status);
    };
  ...
  subscribeOpenDialog: (listener: SankiWorkHostUpdaterOpenDialogListener): (() => void) => {
    const handler = (_event: unknown, request: SankiWorkHostUpdaterOpenDialogRequest): void => {
...
const hostBridge = {
  ...
  updater,
} satisfies SankiWorkHostBridge;

contextBridge.exposeInMainWorld(SANKIWORK_HOST_GLOBAL, hostBridge);

contextBridge.exposeInMainWorld('sankiWorkDesktop', {
