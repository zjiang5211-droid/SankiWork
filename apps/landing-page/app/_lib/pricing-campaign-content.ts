...
export const PRICING_CAMPAIGN_CONTENT_BY_LOCALE = {
  en: {
    ...
    disclaimer: 'Unlimited model quota and free generations included in a plan are available only in SankiWork; they cannot be used through MCP/CLI/API or in other scenarios. Some models may require queuing during peak hours. The organizer reserves the right of final interpretation.',
  ...
  zh: {
    ...
    disclaimer: '套餐内的无限制模型额度与免费生成次数，仅可通过SankiWork使用；无法在MCP/CLI/API及其他场景使用。部分模型高峰期需要排队。解释权归官方所有。',
  ...
  'zh-tw': {
    ...
    disclaimer: '方案內的無限制模型額度與免費生成次數，僅可透過 SankiWork 使用；無法在 MCP/CLI/API 及其他場景使用。部分模型在高峰時段可能需要排隊。最終解釋權歸官方所有。',
  ...
  ja: {
    ...
    disclaimer: 'プランに含まれる無制限のモデル枠と無料生成回数は、SankiWork内でのみ利用できます。MCP/CLI/APIなど、その他の環境では利用できません。一部のモデルはピーク時に待ち時間が発生する場合があります。最終的な解釈権は運営者に帰属します。',
  ...
  ko: {
    ...
    disclaimer: '플랜에 포함된 무제한 모델 한도와 무료 생성 횟수는 SankiWork에서만 사용할 수 있으며 MCP/CLI/API 또는 기타 환경에서는 사용할 수 없습니다. 일부 모델은 피크 시간대에 대기해야 할 수 있습니다. 최종 해석 권한은 운영사에 있습니다.',
  ...
  ru: {
    ...
    disclaimer: 'Безлимитная квота моделей и бесплатные генерации, включённые в тариф, доступны только в SankiWork. Они недоступны через MCP/CLI/API или в других сценариях. Для некоторых моделей в часы пик может потребоваться ожидание в очереди. Организатор оставляет за собой право окончательного толкования.',
  ...
  es: {
    ...
    disclaimer: 'La cuota ilimitada de modelos y las generaciones gratuitas incluidas en el plan solo pueden utilizarse en SankiWork, no mediante MCP/CLI/API ni en otros entornos. Algunos modelos pueden requerir espera en horas punta. El organizador se reserva el derecho de interpretación final.',
  ...
  'pt-br': {
    ...
    disclaimer: 'A cota ilimitada de modelos e as gerações gratuitas incluídas no plano só podem ser usadas no SankiWork, e não via MCP/CLI/API nem em outros cenários. Alguns modelos podem exigir espera em horários de pico. O organizador se reserva o direito de interpretação final.',
  ...
  it: {
    ...
    disclaimer: 'La quota modelli illimitata e le generazioni gratuite incluse nel piano sono utilizzabili solo in SankiWork, non tramite MCP/CLI/API né in altri contesti. Alcuni modelli potrebbero richiedere attesa nelle ore di punta. L’organizzatore si riserva il diritto di interpretazione finale.',
  ...
  tr: {
    ...
    disclaimer: 'Paket kapsamındaki sınırsız model kotası ve ücretsiz üretim hakları yalnızca SankiWork içinde kullanılabilir; MCP/CLI/API veya diğer senaryolarda kullanılamaz. Bazı modeller yoğun saatlerde sıraya alınabilir. Nihai yorum hakkı organizatöre aittir.',
