...
const FAQ_EN: FaqItem[] =
  [
    { q: "What are SankiWork model credits? How are they used?", a: "SankiWork model credits are the USD-denominated balance used for SankiWork hosted models. Each hosted AI model call deducts from your SankiWork balance by usage. BYOK calls to provider models do not consume credits. Each subscription grants the matching credit amount every billing cycle." },
    { q: "How do the plans differ? Can Free use hosted models?", a: "Free does not include a recurring SankiWork model-credit grant, but every new sign-up receives a limited-time trial credit that unlocks the trial pool of hosted models (currently Grok-4.5) for 7 days. After the trial credit expires, Free goes back to local CLI plus BYOK. Plus, Pro, and Max grant the model-credit amounts shown on their plan cards every billing cycle for different usage levels. Every plan, including Free, supports BYOK.", aTrialOff: "Free does not include a recurring SankiWork model-credit grant and supports local CLI plus BYOK only. Plus, Pro, and Max grant the model-credit amounts shown on their plan cards every billing cycle for different usage levels. Every plan, including Free, supports BYOK." },
    { q: "What is the Free plan's limited-time trial credit?", a: "The limited-time trial credit is a free welcome benefit SankiWork Cloud grants every new sign-up: with zero deployment or setup, you can start generating high-quality design deliverables right away. The credit amount is limited, and it expires 7 days after it is granted — or as soon as you subscribe to a paid plan.", trialPromo: true },
    ...
    { q: "What happens when monthly credits run out? Do they roll over?", a: "SankiWork model credits are granted by billing cycle. They reset each cycle and unused credits do not roll over. When credits run out, you can upgrade, enable auto top-up, or use BYOK provider keys to keep calling models without spending SankiWork credits." },
    { q: "How does auto top-up work?", a: "After authorization, you set a monthly top-up cap. When the SankiWork balance is low, the system adds credits and charges your saved payment method. Total auto top-ups in a calendar month will not exceed your cap. You can update the cap or turn it off in Wallet at any time." },
    { q: "What is BYOK? Do all plans support it?", a: "BYOK means Bring Your Own Key: add your own Anthropic, OpenAI, Google, or other provider API key and call provider models directly. BYOK does not consume SankiWork balance and is available on every plan, including Free." },
...
const FAQ_ZH: FaqItem[] =
  [
    { q: "SankiWork 模型额度是什么？怎么消耗？", a: "SankiWork 模型额度是用于调用 SankiWork 托管模型的余额，以美元计。每次调用 SankiWork 托管的 AI 模型时，按用量从你的 SankiWork 余额中扣减；使用 BYOK（自带 API Key）调用原厂模型不消耗额度。订阅后每个计费周期发放对应金额的额度。" },
    { q: "各套餐有什么区别？Free 能用托管模型吗？", a: "Free 不含按周期发放的 SankiWork 模型额度，但新注册用户会获得一笔限时体验额度，7 天内可调用体验模型池中的托管模型（当前为 Grok-4.5）；限时额度过期后，Free 回到本地 CLI 与 BYOK 自带密钥。Plus / Pro / Max 每个计费周期发放的模型额度以套餐卡片当前显示为准，适用从个人轻度到团队高频的不同强度。所有套餐（含 Free）都支持 BYOK。", aTrialOff: "Free 不含按周期发放的 SankiWork 模型额度，仅支持本地 CLI 与 BYOK 自带密钥。Plus / Pro / Max 每个计费周期发放的模型额度以套餐卡片当前显示为准，适用从个人轻度到团队高频的不同强度。所有套餐（含 Free）都支持 BYOK。" },
    { q: "Free 的限时额度是什么？", a: "Free 的限时额度是 SankiWork Cloud 为新注册用户提供的免费体验福利：无需部署、无需配置，注册后即可直接生成高质量的设计产物。限时额度数量有限，发放后 7 天过期；订阅任意套餐后，剩余的限时额度也会随之失效。", trialPromo: true },
    ...
    { q: "月度额度用完了怎么办？会累积到下月吗？", a: "套餐的 SankiWork 模型额度按计费周期发放，每个周期重置、未用完不结转到下个周期。额度用完后可以：升级到更高套餐、开启自动充值在余额不足时自动补充，或改用 BYOK 自带密钥继续调用（不消耗额度）。" },
    { q: "自动充值是怎么工作的？", a: "授权后由你设定每月充值上限（这是唯一需要你设置的值）。当账户 SankiWork 余额不足时，系统会自动补充额度并从你绑定的支付方式扣款，单个自然月内的累计自动充值金额不会超过你设定的上限；触发时机与每次充值金额由系统自动管理。可随时在钱包中调整上限或关闭，关闭后不再自动扣款。" },
    { q: "BYOK 是什么？所有套餐都支持吗？", a: "BYOK（Bring Your Own Key）是指填入你自己的 Anthropic、OpenAI、Google 等厂商 API Key，直接调用原厂模型，不消耗 SankiWork 余额、也不受套餐模型列表限制。所有套餐（包括 Free）均支持 BYOK。" },
...
const FAQ_ZH_TW: FaqItem[] =
  [
    { q: "SankiWork 模型額度是什麼？如何消耗？", a: "SankiWork 模型額度是用於調用 SankiWork 托管模型的美元餘額。每次調用托管 AI 模型時，會按用量從餘額扣減；使用 BYOK 調用原廠模型不消耗額度。訂閱後每個計費週期會發放對應額度。" },
    { q: "各套餐有什麼差異？Free 能使用托管模型嗎？", a: "Free 不含按週期發放的 SankiWork 模型額度，但新註冊用戶會獲得一筆限時體驗額度，7 天內可調用體驗模型池中的托管模型（目前為 Grok-4.5）；限時額度過期後，Free 回到本地 CLI 與 BYOK。Plus、Pro、Max 每個計費週期發放的模型額度以方案卡片目前顯示為準，適用不同使用強度。所有套餐（含 Free）都支援 BYOK。", aTrialOff: "Free 不含按週期發放的 SankiWork 模型額度，僅支援本地 CLI 與 BYOK。Plus、Pro、Max 每個計費週期發放的模型額度以方案卡片目前顯示為準，適用不同使用強度。所有套餐（含 Free）都支援 BYOK。" },
    { q: "Free 的限時額度是什麼？", a: "Free 的限時額度是 SankiWork Cloud 為新註冊用戶提供的免費體驗福利：無需部署、無需配置，註冊後即可直接生成高品質的設計產物。限時額度數量有限，發放後 7 天過期；訂閱任意方案後，剩餘的限時額度也會隨之失效。", trialPromo: true },
    ...
    { q: "月度額度用完怎麼辦？會結轉嗎？", a: "SankiWork 模型額度按計費週期發放，每個週期重置，未用完不會結轉。額度用完後可以升級、開啟自動充值，或使用 BYOK 原廠密鑰繼續調用模型。" },
    { q: "自動充值如何運作？", a: "授權後你可以設定每月充值上限。當 SankiWork 餘額偏低時，系統會自動補充額度並向已綁定的支付方式扣款；單個自然月內自動充值總額不會超過上限。你可以隨時在錢包中調整或關閉。" },
    { q: "BYOK 是什麼？所有套餐都支援嗎？", a: "BYOK 指 Bring Your Own Key：填入自己的 Anthropic、OpenAI、Google 等廠商 API Key，直接調用原廠模型。BYOK 不消耗 SankiWork 餘額，所有套餐（含 Free）都支援。" },
...
const FAQ_PT_BR: FaqItem[] =
  [
    ...
    { q: "O que acontece quando os créditos acabam? Eles acumulam?", a: "Créditos são concedidos por ciclo de cobrança, reiniciam a cada ciclo e não acumulam. Ao acabar, você pode fazer upgrade, ativar recarga automática ou usar chaves BYOK para continuar chamando modelos sem gastar créditos SankiWork." },
    { q: "Como funciona a recarga automática?", a: "Após autorizar, você define um limite mensal. Quando o saldo SankiWork fica baixo, o sistema adiciona créditos e cobra o método de pagamento salvo. O total mensal não passa do limite. Você pode ajustar ou desligar no Wallet." },
    { q: "O que é BYOK? Todos os planos suportam?", a: "BYOK significa Bring Your Own Key: use sua chave Anthropic, OpenAI, Google ou de outro provedor para chamar modelos diretamente. BYOK não consome saldo SankiWork e está disponível em todos os planos." },
...
const FAQ_ES: FaqItem[] =
  [
    { q: "¿Qué son los créditos de modelo SankiWork? ¿Cómo se usan?", a: "Son el saldo en USD usado para modelos alojados por SankiWork. Cada llamada a un modelo alojado descuenta del saldo según el uso. Las llamadas BYOK a modelos de proveedor no consumen créditos. Cada suscripción concede créditos en cada ciclo de facturación." },
    { q: "¿En qué se diferencian los planes? ¿Free usa modelos alojados?", a: "Free no incluye créditos de modelo SankiWork recurrentes, pero cada nuevo usuario recibe un crédito de prueba por tiempo limitado que desbloquea el grupo de prueba de modelos alojados (actualmente Grok-4.5) durante 7 días. Al caducar ese crédito, Free vuelve a admitir solo CLI local con BYOK. Plus, Pro y Max conceden en cada ciclo de facturación los créditos mostrados en sus tarjetas para distintos niveles de uso. Todos los planes, incluido Free, admiten BYOK.", aTrialOff: "Free no incluye créditos de modelo SankiWork recurrentes y solo admite CLI local con BYOK. Plus, Pro y Max conceden en cada ciclo de facturación los créditos mostrados en sus tarjetas para distintos niveles de uso. Todos los planes, incluido Free, admiten BYOK." },
    { q: "¿Qué es el crédito por tiempo limitado de Free?", a: "El crédito por tiempo limitado es un beneficio de bienvenida que SankiWork Cloud concede a cada nuevo usuario: sin despliegues ni configuración, puedes empezar a generar entregables de diseño de alta calidad nada más registrarte. La cantidad es limitada y el crédito caduca 7 días después de concederse, o en cuanto te suscribes a un plan de pago.", trialPromo: true },
    ...
    { q: "¿Qué pasa si se agotan los créditos? ¿Se acumulan?", a: "Los créditos se conceden por ciclo de facturación, se reinician en cada ciclo y no se acumulan. Al agotarse, puedes subir de plan, activar recarga automática o usar claves BYOK para seguir llamando modelos sin gastar créditos SankiWork." },
    { q: "¿Cómo funciona la recarga automática?", a: "Tras autorizarla, defines un límite mensual. Cuando el saldo SankiWork baja, el sistema añade créditos y cobra el método de pago guardado. El total mensual no supera tu límite. Puedes cambiarlo o desactivarlo en Wallet." },
    { q: "¿Qué es BYOK? ¿Todos los planes lo admiten?", a: "BYOK significa Bring Your Own Key: usa tu propia clave de Anthropic, OpenAI, Google u otro proveedor para llamar modelos directamente. BYOK no consume saldo SankiWork y está disponible en todos los planes." },
...
const FAQ_RU: FaqItem[] =
  [
    { q: "Что такое кредиты моделей SankiWork и как они расходуются?", a: "Кредиты моделей SankiWork — это баланс в USD для моделей, размещенных SankiWork. Каждый вызов размещенной модели списывает баланс по фактическому использованию. Вызовы BYOK к моделям провайдеров кредиты не тратят. Каждая подписка выдает кредиты в каждом расчетном цикле." },
    { q: "Чем отличаются планы? Может ли Free использовать размещенные модели?", a: "Free не включает регулярную выдачу кредитов моделей SankiWork, но каждый новый пользователь получает временный пробный кредит, который на 7 дней открывает пробный пул размещенных моделей (сейчас это Grok-4.5). После истечения пробного кредита Free возвращается к локальному CLI с BYOK. Plus, Pro и Max выдают в каждом расчетном цикле кредиты, показанные на карточках планов, для разных уровней использования. Все планы, включая Free, поддерживают BYOK.", aTrialOff: "Free не включает регулярную выдачу кредитов моделей SankiWork и поддерживает только локальный CLI с BYOK. Plus, Pro и Max выдают в каждом расчетном цикле кредиты, показанные на карточках планов, для разных уровней использования. Все планы, включая Free, поддерживают BYOK." },
    { q: "Что такое временные кредиты плана Free?", a: "Временные кредиты — это приветственный бонус, который SankiWork Cloud выдает каждому новому пользователю: без развертывания и настройки вы сразу после регистрации можете создавать дизайн-результаты высокого качества. Объем кредитов ограничен; они истекают через 7 дней после выдачи или сразу после оформления платной подписки.", trialPromo: true },
    ...
    { q: "Что будет, когда месячные кредиты закончатся? Они переносятся?", a: "Кредиты выдаются по расчетным циклам, сбрасываются каждый цикл и не переносятся. Когда они заканчиваются, можно повысить план, включить автопополнение или использовать ключи BYOK для вызова моделей без расхода кредитов SankiWork." },
    { q: "Как работает автопополнение?", a: "После авторизации вы задаете месячный лимит. Когда баланс SankiWork становится низким, система добавляет кредиты и списывает оплату с сохраненного метода. Сумма автопополнений за месяц не превысит лимит. Лимит можно изменить или отключить в Wallet." },
    { q: "Что такое BYOK? Все планы это поддерживают?", a: "BYOK означает Bring Your Own Key: добавьте собственный ключ Anthropic, OpenAI, Google или другого провайдера и вызывайте модели напрямую. BYOK не расходует баланс SankiWork и доступен во всех планах." },
...
const FAQ_FR: FaqItem[] =
  [
    ...
    { q: "Que se passe-t-il quand les crédits mensuels sont épuisés ? Sont-ils reportés ?", a: "Les crédits sont accordés par cycle de facturation, réinitialisés à chaque cycle et non reportés. Une fois épuisés, vous pouvez passer à un plan supérieur, activer la recharge automatique ou utiliser BYOK pour continuer sans consommer de crédits SankiWork." },
    { q: "Comment fonctionne la recharge automatique ?", a: "Après autorisation, vous définissez un plafond mensuel. Quand le solde SankiWork est bas, le système ajoute des crédits et facture le moyen de paiement enregistré. Le total mensuel ne dépasse pas le plafond. Vous pouvez l'ajuster ou la désactiver dans Wallet." },
    { q: "Qu'est-ce que BYOK ? Tous les plans le prennent-ils en charge ?", a: "BYOK signifie Bring Your Own Key : ajoutez votre propre clé Anthropic, OpenAI, Google ou autre fournisseur pour appeler les modèles directement. BYOK ne consomme pas le solde SankiWork et est disponible sur tous les plans." },
...
const FAQ_KO: FaqItem[] =
  [
    { q: "SankiWork 모델 크레딧은 무엇이며 어떻게 사용되나요?", a: "SankiWork 모델 크레딧은 SankiWork 호스팅 모델에 사용하는 USD 기준 잔액입니다. 호스팅 AI 모델을 호출할 때 사용량에 따라 잔액에서 차감됩니다. BYOK로 제공자 모델을 호출하는 경우 크레딧을 사용하지 않습니다. 각 구독은 결제 주기마다 해당 크레딧을 지급합니다." },
    { q: "각 플랜은 어떻게 다른가요? Free도 호스팅 모델을 쓸 수 있나요?", a: "Free에는 결제 주기마다 지급되는 SankiWork 모델 크레딧이 없지만, 신규 가입자는 기간 한정 체험 크레딧을 받아 7일 동안 체험 풀의 호스팅 모델(현재 Grok-4.5)을 사용할 수 있습니다. 체험 크레딧이 만료되면 Free는 로컬 CLI와 BYOK만 지원합니다. Plus, Pro, Max는 결제 주기마다 플랜 카드에 표시된 크레딧을 제공해 사용량 수준에 맞게 선택할 수 있습니다. Free를 포함한 모든 플랜은 BYOK를 지원합니다.", aTrialOff: "Free에는 결제 주기마다 지급되는 SankiWork 모델 크레딧이 없으며 로컬 CLI와 BYOK만 지원합니다. Plus, Pro, Max는 결제 주기마다 플랜 카드에 표시된 크레딧을 제공해 사용량 수준에 맞게 선택할 수 있습니다. Free를 포함한 모든 플랜은 BYOK를 지원합니다." },
    { q: "Free의 기간 한정 크레딧이란 무엇인가요?", a: "기간 한정 크레딧은 SankiWork Cloud가 신규 가입자에게 제공하는 무료 체험 혜택입니다. 배포나 설정 없이 가입 직후 바로 고품질 디자인 결과물을 생성할 수 있습니다. 크레딧 수량은 한정되어 있으며, 지급 후 7일이 지나거나 유료 플랜을 구독하면 만료됩니다.", trialPromo: true },
    ...
    { q: "월간 크레딧을 모두 쓰면 어떻게 되나요? 이월되나요?", a: "크레딧은 결제 주기마다 지급되고 주기마다 초기화되며 이월되지 않습니다. 크레딧이 부족하면 상위 플랜으로 업그레이드하거나 자동 충전을 켜거나 BYOK 키로 SankiWork 크레딧 없이 모델을 계속 호출할 수 있습니다." },
    { q: "자동 충전은 어떻게 작동하나요?", a: "승인 후 월간 충전 한도를 설정합니다. SankiWork 잔액이 낮아지면 시스템이 크레딧을 추가하고 저장된 결제 수단으로 청구합니다. 한 달 자동 충전 총액은 한도를 넘지 않습니다. Wallet에서 한도를 조정하거나 끌 수 있습니다." },
    { q: "BYOK란 무엇인가요? 모든 플랜에서 지원하나요?", a: "BYOK는 Bring Your Own Key의 약자로, Anthropic, OpenAI, Google 등 제공자의 API 키를 직접 추가해 모델을 호출하는 방식입니다. BYOK는 SankiWork 잔액을 사용하지 않으며 모든 플랜에서 지원됩니다." },
...
const FAQ_DE: FaqItem[] =
  [
    ...
    { q: "Was passiert, wenn monatliche Credits aufgebraucht sind? Werden sie übertragen?", a: "Credits werden pro Abrechnungszyklus gewährt, in jedem Zyklus zurückgesetzt und nicht übertragen. Wenn sie aufgebraucht sind, kannst du upgraden, Auto-Top-up aktivieren oder BYOK-Schlüssel nutzen, um Modelle ohne SankiWork Credits aufzurufen." },
    { q: "Wie funktioniert Auto-Top-up?", a: "Nach der Autorisierung legst du ein Monatslimit fest. Wenn das SankiWork Guthaben niedrig ist, fügt das System Credits hinzu und belastet die gespeicherte Zahlungsmethode. Die Summe im Kalendermonat überschreitet das Limit nicht. Du kannst es in Wallet ändern oder deaktivieren." },
    { q: "Was ist BYOK? Unterstützen alle Pläne das?", a: "BYOK bedeutet Bring Your Own Key: Du fügst deinen eigenen API-Schlüssel von Anthropic, OpenAI, Google oder anderen Anbietern hinzu und rufst Modelle direkt auf. BYOK verbraucht kein SankiWork Guthaben und ist in allen Plänen verfügbar." },
...
const FAQ_JA: FaqItem[] =
  [
    { q: "SankiWork モデルクレジットとは何ですか？どのように消費されますか？", a: "SankiWork モデルクレジットは、SankiWork がホストするモデルに使う USD 建ての残高です。ホストモデルを呼び出すたびに使用量に応じて差し引かれます。BYOK でプロバイダーモデルを呼び出す場合、クレジットは消費されません。各サブスクリプションは請求サイクルごとに対応するクレジットを付与します。" },
    { q: "各プランの違いは？Free でホストモデルは使えますか？", a: "Free には請求サイクルごとの SankiWork モデルクレジット付与はありませんが、新規登録ユーザーには期間限定クレジットが付与され、7 日間は体験プールのホストモデル（現在は Grok-4.5）を利用できます。期間限定クレジットの失効後、Free はローカル CLI と BYOK のみのサポートに戻ります。Plus、Pro、Max は請求サイクルごとにプランカードに表示されるクレジットを付与し、利用量に応じて選べます。Free を含むすべてのプランで BYOK を利用できます。", aTrialOff: "Free には請求サイクルごとの SankiWork モデルクレジット付与はなく、ローカル CLI と BYOK のみをサポートします。Plus、Pro、Max は請求サイクルごとにプランカードに表示されるクレジットを付与し、利用量に応じて選べます。Free を含むすべてのプランで BYOK を利用できます。" },
    { q: "Free の期間限定クレジットとは何ですか？", a: "期間限定クレジットは、SankiWork Cloud が新規登録ユーザーに提供する無料体験特典です。デプロイや設定は不要で、登録後すぐに高品質なデザイン成果物を生成できます。クレジット数には限りがあり、付与から 7 日後、または有料プランを購読した時点で失効します。", trialPromo: true },
    ...
    { q: "月間クレジットを使い切るとどうなりますか？繰り越されますか？", a: "クレジットは請求サイクルごとに付与され、各サイクルでリセットされます。未使用分は繰り越されません。使い切った場合はアップグレード、自動チャージの有効化、または BYOK キーを使って SankiWork クレジットを使わずにモデル呼び出しを継続できます。" },
    { q: "自動チャージはどのように機能しますか？", a: "承認後、月間チャージ上限を設定します。SankiWork 残高が少なくなると、システムがクレジットを追加し、保存済みの支払い方法に請求します。月間合計は上限を超えません。Wallet で上限変更や停止ができます。" },
    { q: "BYOK とは何ですか？すべてのプランで使えますか？", a: "BYOK は Bring Your Own Key の略で、自分の Anthropic、OpenAI、Google などの API キーを追加してモデルを直接呼び出す方式です。BYOK は SankiWork 残高を消費せず、すべてのプランで利用できます。" },
...
export const ENTERPRISE_EMAIL = 'support@sanki-ai.cloud';
...
const ENTERPRISE_ZH: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "提交失败，请稍后重试，或邮件联系 support@sanki-ai.cloud",
    close: "关闭",
  },
};
...
const ENTERPRISE_EN: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Couldn't submit — please try again or email support@sanki-ai.cloud",
    close: "Close",
  },
};
...
const ENTERPRISE_ZH_TW: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "提交失敗，請稍後重試，或來信 support@sanki-ai.cloud",
    close: "關閉",
  },
};
...
const ENTERPRISE_PT_BR: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Não foi possível enviar. Tente novamente ou escreva para support@sanki-ai.cloud",
    close: "Fechar",
  },
};
...
const ENTERPRISE_ES: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "No se pudo enviar. Inténtalo de nuevo o escribe a support@sanki-ai.cloud",
    close: "Cerrar",
  },
};
...
const ENTERPRISE_RU: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Не удалось отправить. Повторите попытку или напишите на support@sanki-ai.cloud",
    close: "Закрыть",
  },
};
...
const ENTERPRISE_FR: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Échec de l'envoi — réessayez ou écrivez à support@sanki-ai.cloud",
    close: "Fermer",
  },
};
...
const ENTERPRISE_KO: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "제출에 실패했습니다. 다시 시도하거나 support@sanki-ai.cloud로 문의해 주세요.",
    close: "닫기",
  },
};
...
const ENTERPRISE_DE: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "Senden fehlgeschlagen — bitte erneut versuchen oder an support@sanki-ai.cloud schreiben",
    close: "Schließen",
  },
};
...
const ENTERPRISE_JA: EnterpriseCopy = {
  ...
  leadForm: {
    ...
    errorSubmit: "送信に失敗しました。もう一度お試しいただくか、support@sanki-ai.cloud までご連絡ください。",
    close: "閉じる",
  },
};
