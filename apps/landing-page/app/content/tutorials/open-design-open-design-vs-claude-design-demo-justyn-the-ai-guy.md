---
title: 'SankiWork vs Claude Design — Same Prompt, Side by Side'
youtubeId: W2B5JICwmCU
summary: A controlled head-to-head — the exact same prompt run in Claude Design and SankiWork — to see whether the open, local alternative really matches the closed one. Covers the multi-variation feature, installing via your coding agent, and where SankiWork pulls ahead. Based on Justyn The AI Guy's demo.
date: 2026-05-05
category: Demo
...
- **Quality is on par.** Both produce clean, genuinely good designs (a Notion-inspired version, a tech-vibes version, a "go nuts" version). Side by side, the pages look near-identical in polish — which makes sense, since SankiWork can also be powered by Claude.
- **SankiWork's variation page is nicer**, and it generated more screens/states from the same brief.
- **The real difference is what surrounds the output:** SankiWork lets you **change the model** (use Claude, or bring your own key for ChatGPT, DeepSeek, etc.), and adds **image and video templates** (via media providers like GPT Image), neither of which the closed tool offers.

The takeaway: the open one matches on design quality and adds flexibility on top.

![A real prototype generated in SankiWork.](/tutorials/open-design-open-design-vs-claude-design-demo-justyn-the-ai-guy/generated-prototype.webp)
...
Justyn installs SankiWork by simply asking his agent to do it: open a folder, start Claude Code (or Codex), and say *"clone and install this GitHub repo,"* pasting the repo URL. It clones, installs, and starts the app — which runs locally and is **directly connected to your Claude Code account**, so it spends your normal CLI tokens, not the closed tool's separate weekly allowance. Prefer no terminal? There's also a desktop app at [sanki-ai.cloud/download](https://sanki-ai.cloud/download).

The built-in design systems from popular brands are baked in, ready to pick when you build.

## Build something (a dashboard)

Create a project, choose **high fidelity**, and prompt — Justyn builds a social-analytics dashboard. Unlike the closed tool (which just made assumptions), SankiWork asks **clarifying questions** (who's it for, surface, visual tone, key metrics, scope, data style) and offers a **visual direction**, then works through a streamed to-do list to build it. The result is a refined single-page dashboard with well-dialed details and slick charts.

![The HyperFrames motion and video gallery in SankiWork.](/tutorials/open-design-open-design-vs-claude-design-demo-justyn-the-ai-guy/motion-gallery.webp)
...
On the same prompt, yes — the page quality is on par (SankiWork can also run on Claude), and it adds multi-variation output, model choice, and image/video that the closed tool lacks.
