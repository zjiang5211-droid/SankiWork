...
summary: 'An archived Gemini CLI walkthrough from an older SankiWork release, plus the current replacement: configure Google Gemini through BYOK, which runs through the isolated OpenCode adapter.'
date: 2026-05-11
...
This page documents the **Gemini CLI** workflow Sandeep Singh used in [his May 2026 walkthrough](https://www.youtube.com/watch?v=RqjrENimZP4). That direct runtime has since been retired from SankiWork. The archived sections below explain what appears in the recording; they are not current setup instructions.

To use Google Gemini models in the current release, configure **Google Gemini as a BYOK provider**. SankiWork executes configured BYOK providers through its isolated OpenCode adapter; it does not detect or invoke the `gemini` executable.
...
2. Open SankiWork **Settings**, choose the BYOK execution path, select **Google Gemini**, then enter the API key and model.
...
The cost story still depends on the provider plan you bring: SankiWork is free and open source, while Google controls Gemini API quotas and billing.

![The SankiWork workspace.](/tutorials/sankiwork-setup-gemini-cli-free-credits-sandeep-singh/01-workspace.webp)
*The SankiWork workspace — open-source, local-first, and driven by whichever coding agent you connect.*
...
You have three ways to install SankiWork. Pick the one that fits you:
...
To use SankiWork without ever opening the GUI — calling it as a skill or MCP server inside your agent — run:
...
Then, inside the agent, just ask: `Use sankiwork to generate a landing page with a modern minimal design system`.

## Archived workflow: connect Gemini CLI in the older release

> The steps in this section describe the release shown in the embedded video. They do not work in current SankiWork releases; use the BYOK setup above instead.

At the time of the recording, this was the step that made the workflow free. SankiWork drove Gemini CLI and used its account-level allowance.

**Why did the recording use Gemini CLI?** Two reasons, as Sandeep put it: the Gemini models held their own against the rest, and the free tier was generous. Those account-level CLI terms are historical context, not the current SankiWork integration contract.
...
3. **Point that older SankiWork release at it.** In the recorded build, restarting SankiWork made Gemini CLI appear as an available agent for connection testing.
...
SankiWork keeps your work **project-scoped**: each project lives in its own folder, and switching folders switches projects. Inside a project you can create prototypes, live artifacts, slide decks, images, and even video and audio — not just UI.

![The built-in design-systems library.](/tutorials/sankiwork-setup-gemini-cli-free-credits-sandeep-singh/02-design-systems.webp)
*The design-systems library — branded starting points you can preview and snap into any project.*

The built-in design-systems library gives you branded starting points to preview and reuse, so you are not blocked on defining tokens before you can begin. You can also bring your own design system, import an existing Claude Design project, or start from nothing and let SankiWork infer sensible defaults.

![The templates library.](/tutorials/sankiwork-setup-gemini-cli-free-credits-sandeep-singh/03-templates.webp)
*Templates: prototype, slide, image, and video starting points — filter by type and fork to begin.*

The templates library reaches beyond brand systems into prototypes, slides, and both image and video generation. Filter by type and fork any one as your starting point. You can browse the full plugin library on the web at [sanki-ai.cloud/plugins](https://sanki-ai.cloud/plugins/) before installing anything.
...
2. **Write the brief.** Instead of one of the suggested prompts, describe what you want — Sandeep asks for a landing page. You can also attach a screenshot of a layout you like and ask SankiWork to follow that theme.
...
- **The recording signs in to Gemini CLI with a Google account.** Current SankiWork releases use a Gemini API key through BYOK instead.
...
- **Your designs stay with the project folder** — organize work by running SankiWork against the right directory.
- **You do not need a design system to start.** Begin from a built-in one, import a Claude Design project, or let SankiWork infer defaults.

## FAQ

**Is it really free?**
SankiWork is free and open source under Apache-2.0. Gemini API usage depends on Google's current free-tier and billing terms; the video's roughly 2% figure came from the retired direct-CLI workflow.

**Which coding agents does it support?**
21+ agents, including Claude Code, Codex, Cursor, GitHub Copilot, OpenCode, and Qwen. SankiWork detects supported CLIs already installed on your machine and also offers configured BYOK runtimes.

**Do I have to use Gemini CLI?**
No. Current SankiWork releases do not expose Gemini CLI as a direct runtime. Choose a supported local agent, or configure Google Gemini through BYOK.

**Do I need a design system before I can start?**
No. SankiWork ships branded design systems and templates as starting points, and you can import an existing design system or let it infer sensible defaults.
