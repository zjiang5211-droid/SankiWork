---
title: 'Deploy SankiWork to the Cloud with Sealos Skills'
youtubeId: BWVigaUeCVg
summary: 'Based on Sealos''s demo: take SankiWork from a local workspace to a live cloud app. A coding agent spins it up from the GitHub repo, then Sealos Skills analyzes the Docker setup, ports, and environment variables and deploys it to a public URL anyone can open.'
date: 2026-06-15
category: Demo
durationSeconds: 239
author: 'Sealos'
...
SankiWork is great on your own machine — but a local URL only works for you, not your team, clients, or partners. This guide follows [Sealos's demo](https://www.youtube.com/watch?v=BWVigaUeCVg), where a coding agent spins SankiWork up from its GitHub repo and then **Sealos Skills** deploys it to a public cloud URL. Watch the video above for the live run, or read on for the written version.

![The SankiWork workspace running before it goes to the cloud.](/tutorials/open-design-cloud-deployment-sealos-sealos/01-workspace.webp)
*SankiWork running locally: an open-source, agent-native design workspace — the starting point before you take it online.*

## What is SankiWork?

SankiWork is an open-source, agent-native design workspace — an alternative to Claude Design and Figma that runs on top of the coding agent you already use. A few things worth knowing before this walkthrough:
...
## Why move SankiWork to the cloud

When you run SankiWork from source, the workspace opens on a **local URL**. That is perfect for solo work: it launches, you design, you preview the result live. But as the Sealos demo points out, there is a catch — that address is essentially localhost. It works for you, and no one else can reach it.

To share a design with teammates, hand a live prototype to a client, or let external users experience the result, the app needs a **public URL**. That is the whole point of this walkthrough: moving SankiWork from a local workspace into an online service, so that "running locally" becomes just the beginning.

## Spin up SankiWork from the GitHub repo

Rather than reading through installation docs by hand, the demo hands the whole job to a coding agent (Codex here). The idea is simple: point the agent at the [SankiWork GitHub repository](https://github.com/nexu-io/open-design) and let it do the setup work.

- **Give the agent the repo.** Send the GitHub link and ask it to get SankiWork running. It reads the README and quick-start guide, figures out the install steps, and runs them for you.
...
Because SankiWork is open source, the agent can inspect the repo directly: the project structure, how it is meant to run, and what it needs. That is what makes the next step — cloud deployment — an agent task rather than a manual one.
...
![A prototype generated in the SankiWork workspace.](/tutorials/open-design-cloud-deployment-sealos-sealos/generated-prototype.webp)
...
![The SankiWork plugins hub.](/tutorials/open-design-cloud-deployment-sealos-sealos/plugins-hub.webp)
...
This is where the local-URL problem gets solved. In the demo, the next instruction to Codex is simply to use **Sealos Skills** — a skill pack for AI agents — to analyze the SankiWork project and deploy it to **Sealos Cloud**.
...
At this point, SankiWork has gone from a local tool to an online service. The real value isn't just getting the design out of the workspace — it is that a manual, judgment-heavy deployment became something an agent can run repeatedly and reliably. Running locally is just a beginning; being reachable by others is what turns your work into something you can actually share.

## FAQ

**Do I have to use Sealos to run SankiWork?**
No. SankiWork is local-first — running it on your own machine works fine. Sealos is the third-party cloud host used in this demo to make the app reachable at a public URL; you can also self-host it however you prefer, since it is open source under Apache-2.0.
...
The demo uses Codex, but SankiWork runs on top of whichever coding agent you connect — Claude Code, Codex, Gemini, and others — using your own key.
