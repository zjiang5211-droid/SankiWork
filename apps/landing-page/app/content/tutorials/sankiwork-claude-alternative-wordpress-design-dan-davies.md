---
title: 'Free Claude Design Alternative? Testing SankiWork AI'
youtubeId: RvxzTLx-JMM
summary: 'A WordPress developer live-tests SankiWork as a free, open-source Claude Design alternative — building brand guidelines and classroom quest tiles by plugging in his own coding-agent subscription. Based on Dan Davies''s hands-on stream.'
date: 2026-06-03
category: Review
...
This is a real, unscripted test of SankiWork by someone who builds and manages websites for a living but does not consider himself a designer. Dan Davies loved Claude Design when it launched, then ran headfirst into its low usage limits — so he went looking for an open-source alternative he could point at his own AI subscription. Watch the full session in [his live walkthrough](https://www.youtube.com/watch?v=RvxzTLx-JMM), or read the written version below.

![The SankiWork workspace running locally.](/tutorials/sankiwork-claude-alternative-wordpress-design-dan-davies/workspace-home.webp)
*The SankiWork workspace: describe what you want, pick a mode, and let the coding agent you already pay for do the work.*

## What is SankiWork?

SankiWork is an open-source, local-first design platform — an agent-native alternative to Claude Design and Figma. Instead of locking you into one vendor's model and one fixed credit pool, it runs **on top of the coding agent you already use**.
...
## The usage-limit problem SankiWork solves
...
This is the feature Dan keeps coming back to, and it is worth understanding clearly. SankiWork does not ship its own model. In **Settings**, it detects the coding-agent CLIs already installed on your machine and lets you flip between them.
...
The distinction he draws is the important one: he deliberately avoids raw API keys, because pay-per-call credits burn real money fast on iterative design work. Pointing SankiWork at a **subscription** you already pay for — Claude Code's plan, or ChatGPT through Hermes — means you are not stacking a second bill on top. That, more than anything, is why he prefers it to a hosted design tool with its own limits.

![The built-in templates and skills library.](/tutorials/sankiwork-claude-alternative-wordpress-design-dan-davies/03-templates.webp)
...
Dan's first serious task is the kind of thing that normally needs a professional designer's time: a **brand guidelines document** for his WP Odyssey school community. He did not have clear documentation of his logo, icons, fonts, or colours, so he asked SankiWork to create one.
...
Two things stand out from how he describes it. First, SankiWork produced *structured* output — not a loose pile of assets, but a presentable document with a consistent way of thinking. Second, he treated it as a living artifact: "we'll keep working on this to improve it." That is the intended loop — generate a strong first version from a real brand, then refine.
...
![A real prototype generated inside SankiWork.](/tutorials/sankiwork-claude-alternative-wordpress-design-dan-davies/generated-prototype.webp)
*SankiWork tends to present generated work as a real, structured artifact — here, a full rendered design in the preview — rather than a loose bundle of files.*
...
The single clearest takeaway from Dan's session is that **output quality tracks the agent you connect**. The identical task, in the same tool, went from a confused mess with one agent to a clean first draft with a stronger one. SankiWork is the workspace and the structure; the model is the engine.

He also draws a nice comparison to plain image tools: he had tried similar tasks in a generic image generator and it "struggled," whereas SankiWork "seems to have a bit more structure in what it gives you." Asked for graphics, it built a small web page to display and download them — a presentable deliverable, not just raw output.
...
The software is open source under the Apache-2.0 licence and free to run locally. You do not pay SankiWork anything. You only pay for the usage of whichever coding agent you connect — and if you plug in a subscription you already have (like Claude Code's plan), there is no extra design-tool bill on top.
...
No — that is the point Dan stresses most. SankiWork detects the agent CLIs on your machine and lets you use a subscription rather than pay-per-call API credits, which he found burns money fast on iterative work.
...
*This written guide is based on Dan Davies's live test of SankiWork. Watch the full session above, and [subscribe to Dan Davies](https://www.youtube.com/watch?v=RvxzTLx-JMM) for more hands-on WordPress and AI-tooling streams.*
