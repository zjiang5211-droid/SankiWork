...
summary: 'Install and run SankiWork on Windows 11 — the simple native desktop build, plus the from-source route through WSL with Node 24 and pnpm. Run the open-source AI UI generator locally, no cloud subscription. Based on AI Automation''s walkthrough.'
date: 2026-05-05
...
This guide is for Windows users who want to run SankiWork on their own machine — no cloud account, no monthly credits to burn through. It follows the path AI Automation takes in [their Windows install walkthrough](https://www.youtube.com/watch?v=GjPYMNsb9mo), rewritten and brought up to date with the current release so you can follow along command by command. Watch the video above for the live run, or read on for the written version.

![The SankiWork workspace running locally.](/tutorials/sankiwork-windows-wsl-installation-setup-ai-automation/01-workspace.webp)
*SankiWork running locally on Windows: an open-source, agent-native design workspace you fully control.*
...
If you have used Claude Design, the feel will be instantly familiar — SankiWork keeps the same calm aesthetic, then adds capabilities (and your choice of model) on top.

## Before you start

You have three ways to install SankiWork on Windows. Pick the one that fits you:
...
With Ubuntu set up and Node 24 active, you are ready to install SankiWork.

## Step 2 — Install SankiWork
...
```bash
git clone https://github.com/nexu-io/open-design.git ~/sankiwork
cd ~/sankiwork
...
Prefer to skip the GUI and call SankiWork as an MCP server inside your agent? On WSL there is **one extra step first**. Linux already ships `/usr/bin/od` (the octal-dump tool), which shadows SankiWork's own `sw` — so `sw mcp install …` would otherwise fail with "file not found" for `mcp`/`install`. Add a small wrapper that points `sw` at your clone and put it first on `PATH`:

```bash
mkdir -p ~/.local/bin
cat > ~/.local/bin/sw <<'EOF'
#!/usr/bin/env bash
repo="$HOME/sankiwork"     # the ~/sankiwork clone from Option B
cd "$repo" || exit 127
...
hash -r
type -a od                   # SankiWork's od should now win
```
...
The first time the workspace loads, SankiWork scans your environment and shows every supported CLI it found. Because WSL is a real Linux box, it picks up agents installed **inside Ubuntu** — for example OpenCode or Qwen. Click **rescan** if something does not show up yet.
...
SankiWork keeps your work **project-scoped**: when you run it against a folder, the designs you create live inside that project directory. Your first project list will be empty — that is normal.
...
![The built-in design-systems library.](/tutorials/sankiwork-windows-wsl-installation-setup-ai-automation/02-design-systems.webp)
*The design-systems library: pick a real brand look — Airbnb, Airtable, and many more — and SankiWork carries its tokens, colors, and type into your design.*
...
![The templates library.](/tutorials/sankiwork-windows-wsl-installation-setup-ai-automation/03-templates.webp)
*Templates: prototype, slide, image, and video starting points — filter by type and fork to begin.*

You can browse the full library on the web at [sanki-ai.cloud/plugins](https://sanki-ai.cloud/plugins/) before installing anything.
...
- **Install your agents where SankiWork will look.** Running from source in WSL? Install your CLIs inside Ubuntu so the workspace detects them; the desktop app reads your Windows `PATH`.
- **You do not need a design system to start.** Begin from a built-in one, import a ZIP, or let SankiWork infer defaults. Your designs stay with the project folder, so organize work by running against the right directory.
...
21+ agents, including Claude Code, Codex, Cursor, GitHub Copilot, OpenCode, and Qwen. SankiWork detects the CLIs already installed on your machine — inside WSL for the source route, or on your Windows `PATH` for the desktop app.
