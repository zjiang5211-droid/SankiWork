---
title: 'SankiWork: The FREE Open-Source Claude Design Alternative'
youtubeId: 2AdKzkQyKkg
summary: 'Based on Code A Program''s demo: build a full AI-writing-tool landing page in SankiWork — pick a coding agent (even a free one), prompt the design, refine it with on-canvas edits, then export or one-click deploy to Vercel or Cloudflare.'
date: 2026-06-26
category: Demo
durationSeconds: 486
...
SankiWork takes an idea and turns it into prototypes, websites, slides, and even HTML videos — all running locally on your own machine. In this demo, Code A Program installs the desktop app, connects a coding agent, and builds a premium landing page for an AI writing tool called *Draft*, then refines and deploys it without leaving the workspace. Watch the full run in [Code A Program's video](https://www.youtube.com/watch?v=2AdKzkQyKkg), or follow the written walkthrough below.

![The SankiWork workspace running locally.](/tutorials/open-design-free-open-source-alternative-code-a-program/01-workspace.webp)
*SankiWork running locally: an open-source, agent-native design workspace you fully control — not locked to a single model.*

## What is SankiWork?

SankiWork is an open-source, local-first design platform — a free alternative to Claude Design and Figma. Instead of tying you to one model provider, it runs **on top of the coding agent you already use**, so you bring your own agent and your own key.
...
If you have used Claude Design, the feel is instantly familiar — SankiWork keeps the same calm experience, then adds your choice of model on top.

## Step 1 — Choose your agent and model

Download the desktop app from [sanki-ai.cloud](https://sanki-ai.cloud/), install it, and answer the short onboarding questions. That drops you into the main chat interface.
...
The chat has an **attach file** option for reference material, plus a **plugin** section. SankiWork ships with 460+ plugins covering build and test helpers, 3D mockups, mobile and desktop device frames, and more — so if you need 3D elements or a device shell, you add the plugin rather than fight the prompt.

![The SankiWork plugin library.](/tutorials/open-design-free-open-source-alternative-code-a-program/plugin-library.webp)
...
Submit the prompt and SankiWork starts preparing the project. After about a minute it may ask a few **follow-up questions** — answer them for better results, or hit **skip all** and let it continue.

In the demo the landing page itself finishes in about **six minutes**. SankiWork then spends the remaining time running **self-checks** — verifying the structure, reviewing the layout, checking animations, and fixing small issues before marking the build complete. When it is done, click the **present icon** to open the site in a new tab.

![A generated landing page previewed in SankiWork.](/tutorials/open-design-free-open-source-alternative-code-a-program/generated-prototype.webp)
...
- **Open in an editor** like Cursor or VS Code, or in CLI tools like Gemini CLI, Codex, and Claude Code. SankiWork hands over the generated HTML plus its Skills folder, so you can convert the design into a Next.js project — or any framework you like.
...
**Is SankiWork free?** Yes — it is open source under Apache-2.0 and runs locally for free. You only pay for the model/API usage of whichever agent and media providers you connect, and you can use free tiers like Gemini's.
