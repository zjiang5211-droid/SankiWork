---
title: 'Which Coding Agent Should You Run SankiWork With?'
youtubeId: 6yyF5kJEO20
summary: A setup guide focused on the one choice that matters most — which coding agent drives SankiWork. It supports a broad runtime set (Claude Code, Codex, Cursor, OpenCode, Qwen, Copilot, Devin, Hermes, Kimi, Pi, Kiro, and more); here's how to pick one, install from source, and build your first design. Based on Tony Xhepa's walkthrough.
date: 2026-05-03
category: Demo
durationSeconds: 421
author: 'Tony Xhepa'
...
SankiWork doesn't ship its own model — it's an open-source, local-first surface that **runs on whatever supported coding agent you already have**. So the agent you pick *is* your design engine: it determines quality, cost, and speed. The runtime list is broad: **Claude Code, Codex, Devin for Terminal, Cursor, OpenCode, Qwen, GitHub Copilot CLI, Hermes, Kimi, Pi, Kiro — and more.** Whatever you're already running, SankiWork probably supports it, and if you have none installed, you can configure a BYOK provider instead.

## How to pick one

A quick way to choose:

- **Already paying for a coding CLI?** Use it — SankiWork rides on that subscription, so there's no second bill.
...
SankiWork is a Next.js app; the Quick Start is a handful of commands:
...
Then open the local URL it prints (a dynamic port — don't hardcode one). One note Tony hits: it wants **Node 24** — on Node 22 you'll see an "unsupported engine" warning, so install Node 24 first. Prefer no terminal? There's a desktop app at [sanki-ai.cloud/download](https://sanki-ai.cloud/download).
...
SankiWork requires Node 24; on Node 22 you'll see an "unsupported engine" warning. Install Node 24 to clear it.
