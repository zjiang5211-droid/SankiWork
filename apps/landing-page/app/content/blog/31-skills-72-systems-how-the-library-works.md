---
title: "31 skills, 72 systems: how the SankiWork library works"
date: 2026-05-13
category: "Guides"
readingTime: 8
summary: "A walk through the four primitives that make SankiWork composable: skills, systems, adapters, and the daemon. With concrete examples of how a Markdown file becomes a pixel-perfect deliverable."
i18n:
  zh:
    title: "31 个 skill、72 个 system：SankiWork 库是怎么运作的"
    summary: "带你走一遍让 SankiWork 可组合的四个原语：skill、system、adapter 和 daemon。并用具体例子说明一个 Markdown 文件如何变成一份像素级精确的交付物。"
    bodyHtml: |
      <p>从机制上看，SankiWork 就是四个原语层层叠起来：</p>
      <ol>
      ...
      <pre class="astro-code sankiwork-editorial" style="background-color:#f7f1de;color:#15140f; overflow-x: auto;" tabindex="0" data-language="plaintext"><code><span class="line"><span>---</span></span>
      ...
      <p>如果你想新增一个 adapter，那个文件大约是 80 行 TypeScript 加一次 <code>register()</code> 调用。没有要学的 SDK，没有要申请的权限，没有要发布到的中心注册表。你笔记本上早已信任的那个 agent，就成了引擎——SankiWork 从不取代它。（姊妹篇 <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK 设计工作流</a> 走了一遍如何把一个 adapter 指向你自己的密钥。）</p>
      ...
      <h2>为什么用文件，而不是数据库</h2>
      <p>每一个原语——skill、system、adapter——都是一个装着文本文件的文件夹。没有中心数据库。没有「SankiWork 账户」。没有那种必须一直运转、你的工作才能一直运转的托管服务。</p>
  ...
  zh-tw:
    title: "31 個技能、72 個系統：SankiWork 函式庫如何運作"
    summary: "帶你走過讓 SankiWork 可組合的四個基本元件：技能、系統、轉接器，以及 daemon。並用具體範例說明一個 Markdown 檔案如何變成像素級精準的交付成果。"
    bodyHtml: |
      <p>從機制上看，SankiWork 是四個基本元件層層堆疊而成：</p>
      <ol>
      ...
      <p>如果你想新增一個轉接器，那個檔案大約是 80 行 TypeScript，外加單獨一次 <code>register()</code> 呼叫。沒有 SDK 要學、沒有權限要申請、沒有中央登錄要發布。你在筆電上已經信任的那個 agent 成為引擎——SankiWork 從不取代它。（姊妹篇 <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK 設計工作流程</a>會帶你走過如何把一個轉接器指向你自己的金鑰。）</p>
      ...
      <h2>為什麼用檔案，而不是資料庫</h2>
      <p>每一個基本元件——技能、系統、轉接器——都是一個文字檔案的資料夾。沒有中央資料庫。沒有「SankiWork 帳號」。沒有一個必須持續運作、你的工作才能持續運作的代管服務。</p>
  ...
  ja:
    title: "31 のスキル、72 のシステム：SankiWork ライブラリの仕組み"
    summary: "SankiWork を組み合わせ可能にしている 4 つのプリミティブ ── スキル、システム、アダプター、daemon ── を一通り見ていきます。1 つの Markdown ファイルがどのようにピクセル単位で精密な成果物になるのか、具体例とともに。"
    bodyHtml: |
      <p>SankiWork は、機構的には、互いに積み重なった 4 つのプリミティブです。</p>
      <ol>
      ...
      <p>新しいアダプターを追加したい場合、ファイルはおよそ 80 行の TypeScript と 1 回の <code>register()</code> 呼び出しです。学ぶべき SDK も、申請すべき権限も、公開先の中央レジストリもありません。あなたがすでにノート PC で信頼しているのと同じエージェントがエンジンになります ── SankiWork がそれを置き換えることは決してありません。（対になる記事 <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK デザインワークフロー</a>では、アダプターを自分のキーに向ける方法を解説しています。）</p>
      ...
      <p>すべてのプリミティブ ── スキル、システム、アダプター ── はテキストファイルのフォルダです。中央データベースはありません。「SankiWork アカウント」もありません。あなたの作業が動き続けるために動き続けねばならないホスト型サービスもありません。</p>
  ...
  ko:
    title: "스킬 31개, 시스템 72개: SankiWork 라이브러리는 어떻게 작동하는가"
    summary: "SankiWork를 조합 가능하게 만드는 네 가지 기본 요소 — 스킬, 시스템, 어댑터, 그리고 daemon — 을 둘러본다. Markdown 파일 한 장이 어떻게 픽셀 단위로 완성된 결과물이 되는지 구체적인 예시와 함께 살펴본다."
    bodyHtml: |
      <p>SankiWork는 구조적으로 보면 서로 차곡차곡 쌓아 올린 네 가지 기본 요소로 이루어져 있습니다:</p>
      <ol>
      ...
      <p>새 어댑터를 추가하고 싶다면, 그 파일은 대략 80줄의 TypeScript와 단 한 번의 <code>register()</code> 호출입니다. 배워야 할 SDK도, 요청해야 할 권한도, 게시해야 할 중앙 레지스트리도 없습니다. 노트북에서 이미 신뢰하고 있는 바로 그 에이전트가 엔진이 됩니다 — SankiWork는 결코 그것을 대체하지 않습니다. (동반 글 <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK 디자인 워크플로</a>에서 어댑터를 당신의 키에 연결하는 방법을 짚어 봅니다.)</p>
      ...
      <p>모든 기본 요소 — 스킬, 시스템, 어댑터 — 는 텍스트 파일이 담긴 폴더입니다. 중앙 데이터베이스는 없습니다. "SankiWork 계정" 같은 것도 없습니다. 당신의 작업이 계속 작동하려면 계속 작동해야만 하는 호스팅 서비스 같은 것도 없습니다.</p>
  ...
  de:
    title: "31 Skills, 72 Systeme: So funktioniert die SankiWork Bibliothek"
    summary: "Ein Rundgang durch die vier Primitive, die SankiWork komponierbar machen: Skills, Systeme, Adapter und der daemon. Mit konkreten Beispielen, wie aus einer Markdown-Datei ein pixelgenaues Resultat wird."
    bodyHtml: |
      <p>SankiWork besteht mechanisch betrachtet aus vier Primitiven, die übereinander gestapelt sind:</p>
      <ol>
      ...
      <p>Jedes Primitiv — Skills, Systeme, Adapter — ist ein Ordner mit Textdateien. Es gibt keine zentrale Datenbank. Es gibt kein „SankiWork-Konto“. Es gibt keinen gehosteten Dienst, der weiterlaufen muss, damit deine Arbeit weiterläuft.</p>
  ...
  fr:
    title: "31 skills, 72 systems : comment fonctionne la bibliothèque SankiWork"
    summary: "Un parcours des quatre primitives qui rendent SankiWork composable : skills, systems, adaptateurs et daemon. Avec des exemples concrets de la façon dont un fichier Markdown devient un livrable au pixel près."
    bodyHtml: |
      <p>SankiWork, mécaniquement, ce sont quatre primitives empilées les unes sur les autres :</p>
      <ol>
      ...
      <p>Chaque primitive — skills, systems, adaptateurs — est un dossier de fichiers texte. Il n'y a aucune base de données centrale. Il n'y a aucun « compte SankiWork ». Il n'y a aucun service hébergé qui doive continuer à fonctionner pour que votre travail continue de fonctionner.</p>
  ...
  ru:
    title: "31 навык, 72 системы: как устроена библиотека SankiWork"
    summary: "Прогулка по четырём примитивам, которые делают SankiWork компонуемым: навыки, системы, адаптеры и daemon. С конкретными примерами того, как файл Markdown превращается в идеальный до пикселя результат."
    bodyHtml: |
      <p>SankiWork с механической точки зрения — это четыре примитива, выстроенных друг на друге:</p>
      <ol>
      ...
      <p>Если вы хотите добавить новый адаптер, файл — это примерно 80 строк TypeScript и единственный вызов <code>register()</code>. Никакого SDK для изучения, никаких разрешений для запроса, никакого центрального реестра для публикации. Тот самый агент, которому вы уже доверяете на своём ноутбуке, становится движком — SankiWork никогда его не заменяет. (Сопутствующий материал <a href="/blog/byok-design-workflow-claude-codex-qwen/">рабочий процесс дизайна BYOK</a> разбирает, как направить адаптер на ваш собственный ключ.)</p>
      ...
      <p>Каждый примитив — навыки, системы, адаптеры — это папка с текстовыми файлами. Нет центральной базы данных. Нет «аккаунта SankiWork». Нет размещённого сервиса, который должен продолжать работать, чтобы продолжала работать ваша работа.</p>
  ...
  es:
    title: "31 skills, 72 systems: cómo funciona la biblioteca de SankiWork"
    summary: "Un recorrido por las cuatro primitivas que hacen que SankiWork sea componible: skills, systems, adapters y el daemon. Con ejemplos concretos de cómo un archivo Markdown se convierte en un entregable perfecto al píxel."
    bodyHtml: |
      <p>SankiWork es, mecánicamente, cuatro primitivas apiladas una sobre otra:</p>
      <ol>
      ...
      <p>Cada primitiva — skills, systems, adapters — es una carpeta de archivos de texto. No hay base de datos central. No hay «cuenta de SankiWork». No hay servicio alojado que tenga que seguir funcionando para que tu trabajo siga funcionando.</p>
  ...
  pt-br:
    title: "31 skills, 72 sistemas: como funciona a biblioteca do SankiWork"
    summary: "Um passeio pelas quatro primitivas que tornam o SankiWork composável: skills, sistemas, adaptadores e o daemon. Com exemplos concretos de como um arquivo Markdown vira uma entrega perfeita até o último pixel."
    bodyHtml: |
      <p>O SankiWork é, mecanicamente, quatro primitivas empilhadas umas sobre as outras:</p>
      <ol>
      ...
      <p>Toda primitiva — skills, sistemas, adaptadores — é uma pasta de arquivos de texto. Não há banco de dados central. Não há “conta do SankiWork.” Não há serviço hospedado que precise continuar funcionando para que o seu trabalho continue funcionando.</p>
  ...
  it:
    title: "31 skill, 72 sistemi: come funziona la libreria di SankiWork"
    summary: "Una panoramica delle quattro primitive che rendono SankiWork componibile: skill, sistemi, adattatori e il daemon. Con esempi concreti di come un file Markdown diventa un deliverable perfetto al pixel."
    bodyHtml: |
      <p>SankiWork è, meccanicamente, quattro primitive impilate una sull'altra:</p>
      <ol>
      ...
      <p>Ogni primitiva — skill, sistemi, adattatori — è una cartella di file di testo. Non c'è alcun database centrale. Non c'è alcun "account SankiWork". Non c'è alcun servizio ospitato che debba continuare a funzionare perché il tuo lavoro continui a funzionare.</p>
  ...
  vi:
    title: "31 skill, 72 system: thư viện SankiWork hoạt động ra sao"
    summary: "Một chuyến dạo qua bốn nguyên tố cốt lõi khiến SankiWork có thể kết hợp linh hoạt: skill, system, adapter và daemon. Kèm ví dụ cụ thể về cách một tệp Markdown trở thành một sản phẩm chuẩn từng pixel."
    bodyHtml: |
      <p>Về mặt cơ chế, SankiWork là bốn nguyên tố cốt lõi xếp chồng lên nhau:</p>
      <ol>
      ...
      <p>Mỗi nguyên tố — skill, system, adapter — là một thư mục các tệp văn bản. Không có cơ sở dữ liệu trung tâm. Không có “tài khoản SankiWork.” Không có dịch vụ được host nào phải tiếp tục hoạt động để công việc của bạn tiếp tục hoạt động.</p>
  ...
  pl:
    title: "31 umiejętności, 72 systemy: jak działa biblioteka SankiWork"
    summary: "Spacer po czterech prymitywach, które czynią SankiWork komponowalnym: umiejętnościach, systemach, adapterach i daemonie. Z konkretnymi przykładami tego, jak plik Markdown staje się dopracowanym co do piksela produktem."
    bodyHtml: |
      <p>SankiWork to, mechanicznie rzecz biorąc, cztery prymitywy ułożone jeden na drugim:</p>
      <ol>
      ...
      <p>Jeśli chcesz dodać nowy adapter, plik to mniej więcej 80 linii TypeScript i jedno wywołanie <code>register()</code>. Żadnego SDK do nauki, żadnego pozwolenia do uzyskania, żadnego centralnego rejestru, do którego trzeba publikować. Ten sam agent, któremu już ufasz na swoim laptopie, staje się silnikiem — SankiWork nigdy go nie zastępuje. (Tekst towarzyszący <a href="/blog/byok-design-workflow-claude-codex-qwen/">Workflow projektowy BYOK</a> omawia kierowanie adaptera na własny klucz.)</p>
      ...
      <p>Każdy prymityw — umiejętności, systemy, adaptery — to folder plików tekstowych. Nie ma centralnej bazy danych. Nie ma „konta SankiWork”. Nie ma hostowanej usługi, która musi działać, by Twoja praca dalej działała.</p>
  ...
  id:
    title: "31 skill, 72 sistem: cara kerja pustaka SankiWork"
    summary: "Telusuri empat primitif yang membuat SankiWork dapat dikomposisi: skill, sistem, adapter, dan daemon. Dengan contoh konkret bagaimana sebuah file Markdown menjadi deliverable yang pixel-perfect."
    bodyHtml: |
      <p>Secara mekanis, SankiWork adalah empat primitif yang ditumpuk satu di atas yang lain:</p>
      <ol>
      ...
      <p>Jika Anda ingin menambahkan adapter baru, file-nya kira-kira 80 baris TypeScript dan satu panggilan <code>register()</code>. Tidak ada SDK yang harus dipelajari, tidak ada izin yang harus diminta, tidak ada registri pusat untuk dipublikasikan. Agent yang sama yang sudah Anda percayai di laptop Anda menjadi mesinnya — SankiWork tidak pernah menggantikannya. (Tulisan pendamping <a href="/blog/byok-design-workflow-claude-codex-qwen/">alur kerja desain BYOK</a> menelusuri cara mengarahkan sebuah adapter ke key Anda sendiri.)</p>
      ...
      <p>Setiap primitif — skill, sistem, adapter — adalah folder berisi file teks. Tidak ada basis data pusat. Tidak ada “akun SankiWork.” Tidak ada layanan berbasis hosting yang harus terus berjalan agar pekerjaan Anda terus berjalan.</p>
  ...
  nl:
    title: "31 skills, 72 systemen: hoe de SankiWork-bibliotheek werkt"
    summary: "Een rondleiding langs de vier primitieven die SankiWork samenstelbaar maken: skills, systemen, adapters en de daemon. Met concrete voorbeelden van hoe een Markdown-bestand een pixel-perfect eindproduct wordt."
    bodyHtml: |
      <p>SankiWork is, mechanisch gezien, vier primitieven die op elkaar zijn gestapeld:</p>
      <ol>
      ...
      <p>Als je een nieuwe adapter wilt toevoegen, is het bestand ongeveer 80 regels TypeScript en één <code>register()</code>-aanroep. Geen SDK om te leren, geen toestemming om aan te vragen, geen centraal register om naar te publiceren. Dezelfde agent die je al vertrouwt op je laptop wordt de motor — SankiWork vervangt hem nooit. (Het bijbehorende stuk <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK-ontwerpworkflow</a> behandelt het richten van een adapter op je eigen sleutel.)</p>
      ...
      <p>Elk primitief — skills, systemen, adapters — is een map met tekstbestanden. Er is geen centrale database. Er is geen “SankiWork-account.” Er is geen gehoste service die moet blijven werken opdat jouw werk blijft werken.</p>
  ...
  ar:
    title: "31 مهارة، 72 نظامًا: كيف تعمل مكتبة SankiWork"
    summary: "جولة عبر العناصر الأساسية الأربعة التي تجعل SankiWork قابلًا للتركيب: المهارات، والأنظمة، والمُحوّلات، والـ daemon. مع أمثلة ملموسة عن كيف يتحوّل ملف Markdown إلى مُنتَج تسليمي مثالي على مستوى البكسل."
    bodyHtml: |
      <p>SankiWork، من الناحية الميكانيكية، هو أربعة عناصر أساسية مُكدّسة فوق بعضها بعضًا:</p>
      <ol>
      ...
      <p>إذا أردت إضافة مُحوّل جديد، فالملف نحو 80 سطرًا من TypeScript واستدعاء <code>register()</code> واحد. لا SDK لتتعلّمه، ولا إذن لتطلبه، ولا سجلّ مركزي لتنشر إليه. العميل نفسه الذي تثق به فعلًا على حاسوبك المحمول يصبح المحرّك — SankiWork لا يستبدله أبدًا. (تتناول القطعة المُرافقة <a href="/blog/byok-design-workflow-claude-codex-qwen/">سير عمل التصميم بنظام BYOK</a> توجيه مُحوّل إلى مفتاحك الخاص.)</p>
      ...
      <p>كل عنصر أساسي — المهارات، والأنظمة، والمُحوّلات — هو مجلّد من ملفات نصّية. لا توجد قاعدة بيانات مركزية. لا يوجد «حساب SankiWork». لا توجد خدمة مُستضافة يجب أن تظلّ تعمل كي يظلّ عملك يعمل.</p>
  ...
  tr:
    title: "31 beceri, 72 sistem: SankiWork kütüphanesi nasıl çalışır"
    summary: "SankiWork'ı birleştirilebilir kılan dört ilkel öğenin incelemesi: beceriler, sistemler, adaptörler ve daemon. Bir Markdown dosyasının nasıl piksel mükemmelliğinde bir teslimata dönüştüğüne dair somut örneklerle."
    bodyHtml: |
      <p>SankiWork, mekanik olarak, birbirinin üzerine yığılmış dört ilkel öğedir:</p>
      <ol>
      ...
      <p>Yeni bir adaptör eklemek isterseniz, dosya kabaca 80 satır TypeScript ve tek bir <code>register()</code> çağrısından ibarettir. Öğrenilecek bir SDK, istenecek bir izin, yayımlanacak merkezi bir kayıt defteri yoktur. Dizüstü bilgisayarınızda zaten güvendiğiniz aynı ajan motor olur — SankiWork onu asla değiştirmez. (Tamamlayıcı yazı <a href="/blog/byok-design-workflow-claude-codex-qwen/">BYOK tasarım iş akışı</a>, bir adaptörü kendi anahtarınıza yöneltme konusunu inceler.)</p>
      ...
      <p>Her ilkel öğe — beceriler, sistemler, adaptörler — bir metin dosyası klasörüdür. Merkezi bir veritabanı yoktur. Bir "SankiWork hesabı" yoktur. Çalışmanızın çalışmaya devam etmesi için çalışmaya devam etmesi gereken barındırılan bir hizmet yoktur.</p>
  ...
  uk:
    title: "31 навичка, 72 системи: як працює бібліотека SankiWork"
    summary: "Огляд чотирьох примітивів, які роблять SankiWork компонованим: навичок, систем, адаптерів і daemon. З конкретними прикладами того, як файл Markdown стає піксельно точним результатом."
    bodyHtml: |
      <p>Механічно SankiWork — це чотири примітиви, складені один на одного:</p>
      <ol>
      ...
      <p>Кожен примітив — навички, системи, адаптери — це тека з текстовими файлами. Немає центральної бази даних. Немає «акаунта SankiWork». Немає хмарного сервісу, який має продовжувати працювати, щоб ваша робота продовжувала працювати.</p>
...
SankiWork is, mechanically, four primitives stacked on top of each other:
...
Every primitive — skills, systems, adapters — is a folder of text files. There is no central database. There is no "SankiWork account." There is no hosted service that has to keep working for your work to keep working.
