...
test('campaign attribution preserves first touch and records the checkout touch separately', () => {
  assert.match(tracker, /inbound\.get\('sw_entry_id'\)/);
  assert.match(tracker, /inbound\.get\('sw_entry_source'\)/);
  ...
  assert.match(tracker, /sw_conversion_source/);
  assert.match(tracker, /sw_campaign_id/);
});

test('campaign attribution carries the consented device id across Pricing → Cloud', () => {
  // Desktop badge stamps sw_device_id; Pricing must not drop it on the next hop.
  assert.match(tracker, /inbound\.get\('sw_device_id'\)/);
  assert.match(tracker, /device_id:\s*inboundDeviceId/);
  assert.match(tracker, /attribution\.device_id/);
  assert.match(tracker, /searchParams\.set\('sw_device_id'/);
});
...
test('campaign membership is explicit on each click, not inherited from a stale inbound id', () => {
  ...
  // inbound sw_campaign_id and re-attach the closed campaign to the payment.
  assert.match(tracker, /campaign_id:\s*campaignId\s*\|\|\s*undefined/);
  assert.doesNotMatch(
    tracker,
    /campaign_id:\s*campaignId\s*\|\|\s*\(inbound\s*&&\s*inbound\.get\('sw_campaign_id'\)\)/,
  );
});
