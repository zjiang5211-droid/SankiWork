...
describe("resolvePackagedMcpBootstrapLaunch", () => {
  it("uses macOS open against the stable signed app bundle", () => {
    expect(resolvePackagedMcpBootstrapLaunch({
      currentExecutablePath:
        "/private/payload/SankiWork.app/Contents/MacOS/SankiWork",
      installedLaunchPath: "/Applications/SankiWork.app",
      platform: "darwin",
    })).toEqual({
  ...
  it("invokes a non-macOS installed launcher directly", () => {
    expect(resolvePackagedMcpBootstrapLaunch({
      currentExecutablePath: "/tmp/payload/sankiwork",
      installedLaunchPath: "/opt/sankiwork/sankiwork",
      platform: "linux",
    })).toEqual({
      command: "/opt/sankiwork/sankiwork",
      args: ["--headless"],
    });
  });
});
