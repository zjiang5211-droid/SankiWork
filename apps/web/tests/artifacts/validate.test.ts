...
describe('validateHtmlArtifact', () => {
  ...
  it('rejects unquoted URL attributes that reference reserved storage', () => {
    const html = '<!doctype html><html><body><img src=./.sankiwork/thumb.png alt="Preview"><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(false);
  });
  ...
  it('rejects CSS import references to reserved storage', () => {
    const html = '<!doctype html><html><head><style>@import "/.sankiwork/foo.css";@import url(./.tmp/theme.css);</style></head><body><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(false);
  });
  ...
  it('accepts plain text mentions of reserved directory names', () => {
    const html = '<!doctype html><html><body><p>The .sankiwork folder and .tmp files are mentioned as documentation text only, not linked paths.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });

  it('accepts external URLs with reserved-looking path segments', () => {
    const html = '<!doctype html><html><body><a href="https://example.test/.sankiwork/reference.html">External docs</a><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });

  it('accepts local URLs that only mention reserved paths in query or hash', () => {
    const html = '<!doctype html><html><head><style>.card{background-image:url("/docs?example=/.sankiwork/ref")}</style></head><body><a href="/docs?example=/.sankiwork/ref">Query docs</a><a href="/docs#/.tmp/ref">Hash docs</a><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });
  ...
  it('accepts text-node mentions of HTML attribute syntax for reserved names', () => {
    const html = '<!doctype html><html><body><p>Documentation can mention examples like href="/.sankiwork/reference.html" without linking to project storage.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });

  it('accepts data URLs with reserved-looking payload text', () => {
    const html = '<!doctype html><html><body><img src="data:text/plain,/.sankiwork/foo" alt="Inline payload"><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });

  it('accepts data URLs with reserved-looking payload text in srcset', () => {
    const html = '<!doctype html><html><body><img srcset="data:text/plain,/.sankiwork/foo 1x" alt="Inline payload"><p>Enough content to look like a real document.</p></body></html>';
    const result = validateHtmlArtifact(html);
    expect(result.ok).toBe(true);
  });
