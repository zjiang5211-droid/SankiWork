// Trigger prompt for the post-completion "Share to SankiWork" submission
...
//
// The bundled scenario at `plugins/_official/scenarios/sw-share-to-community/`
...
export const SHARE_TO_COMMUNITY_PROMPT = [
  'Package the work I just finished in this project as an SankiWork plugin so I can install it into My plugins and open a PR to the SankiWork community catalog.',
  '',
  'Follow the bundled `sw-share-to-community` scenario at `plugins/_official/scenarios/sw-share-to-community/SKILL.md` end to end. Do not ask me for fields the project files already answer — read `*.artifact.json`, `brand-spec.md` / `DESIGN.md`, and the generated artifacts in this workspace, then derive the manifest from what you find.',
  '',
  ...
  '',
  'Validate the plugin locally before reporting: run `sw plugin validate` on the folder, then `sw plugin pack` for a tarball, then `sw plugin install --source <absolute-folder-path>` to confirm the install path works.',
  '',
  'When the work above is done, write a single summary turn covering: files created, `sw plugin validate` status, local install / run status, and `sw plugin pack` output. Then STOP.',
  '',
