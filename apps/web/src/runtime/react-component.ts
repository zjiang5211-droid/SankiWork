    ...
    <script>
      (function(){
        ...
        try {
          ...
          (0, eval)(compiled);
          var Component = window.__SankiWorkComponent ||
...
window.__SankiWorkComponent = window.__SankiWorkComponent || (${componentFallbackExpression(transformed.defaultName)});`;
}
...
function transformExports(source: string): { code: string; defaultName: string | null } {
  ...
  code = code.replace(
    ...
    (_match, name: string | undefined) => {
      defaultName = name || 'SankiWorkComponent';
  ...
  code = code.replace(/export\s+default\s+/g, () => {
    defaultName = 'SankiWorkComponent';
    return 'const SankiWorkComponent = ';
  });
  code = code.replace(
