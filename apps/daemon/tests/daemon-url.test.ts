...
import { createJsonIpcServer, type JsonIpcServerHandle } from "@sankiwork/sidecar";
...
// Verifies the resolution chain: --daemon-url > SW_DAEMON_URL > sidecar
// IPC status discovery > legacy default. Each layer must short-circuit the next
// so `sw` clients follow the live daemon across ephemeral-port restarts.
...
describe("resolveDaemonUrl", () => {
  ...
  it("prefers the explicit --daemon-url flag", async () => {
    const url = await resolveDaemonUrl({
      ...
      env: {
        SW_DAEMON_URL: "http://env.example:2222",
  ...
  it("falls back to SW_DAEMON_URL when no flag given", async () => {
    const url = await resolveDaemonUrl({
      env: {
        SW_DAEMON_URL: "http://env.example:2222",
        [SIDECAR_ENV.IPC_PATH]: path.join(ipcBaseDir, "daemon.sock"),
  ...
  it("discovers the live daemon URL via the concrete sidecar IPC status endpoint", async () => {
    const socketPath = process.platform === "win32"
      ? `\\\\.\\pipe\\sankiwork-daemon-url-${process.pid}-${Date.now()}`
