...
describe('senseaudio media generation', () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), 'sw-senseaudio-'));
    ...
    delete process.env.SW_SENSEAUDIO_API_KEY;
  ...
  it('reads the API key from SW_SENSEAUDIO_API_KEY when storage is empty', async () => {
    process.env.SW_SENSEAUDIO_API_KEY = 'env-sense-key';
    const fetchMock = vi.fn(async (_input: unknown, init?: RequestInit) => {
      expect(init?.headers).toMatchObject({
