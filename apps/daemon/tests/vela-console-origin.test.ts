...
// packaged builds at build time (SW_VELA_WEB_URL) and reported to the client
...
describe('resolveVelaConsoleOrigin', () => {
  it('reports the configured origin with any trailing slash removed', () => {
    expect(
      resolveVelaConsoleOrigin({ SW_VELA_WEB_URL: 'https://vela.example.invalid' }),
  ...
  it('reports nothing when the runtime was never given an origin', () => {
    ...
    expect(resolveVelaConsoleOrigin({ SW_VELA_WEB_URL: '   ' })).toBeUndefined();
  });
});
