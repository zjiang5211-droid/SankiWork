...
const originalMode = process.env.SW_DSH_SETUP_FAKE_MODE;
...
afterEach(async () => {
  ...
  if (originalMode === undefined) delete process.env.SW_DSH_SETUP_FAKE_MODE;
  else process.env.SW_DSH_SETUP_FAKE_MODE = originalMode;
  if (originalAgentHome === undefined) delete process.env.SW_AGENT_HOME;
  else process.env.SW_AGENT_HOME = originalAgentHome;
...
async function fixture(options: { existingProfile?: boolean; validHash?: boolean } = {}) {
  ...
  const tarball = path.join(bundleRoot, 'sankiwork-dsh-runtime-0.1.0.tgz');
  ...
  if (options.existingProfile) {
    const profileRoot = path.join(dshHome, 'profiles', 'sankiwork');
...
const profileRoot = path.join(home, 'profiles', 'sankiwork');
...
if (args[0] === '--version') {
  ...
  try {
    ...
    process.stdout.write(JSON.stringify({v:1,type:'probe',runtime:'sankiwork',protocol_version:1,plugin_version:'0.1.0',capabilities:{session_resume:true,session_cancel:true,structured_events:true}}) + '\\n');
  } catch {
    process.exitCode = 1;
  }
} else if (args[0] === 'plugin' && args[1] === '--profile' && args[2] === 'sankiwork' && args[3] === 'add') {
  if (process.env.SW_DSH_SETUP_FAKE_MODE === 'install-fail') process.exit(7);
  if (process.env.SW_DSH_SETUP_FAKE_MODE === 'require-profile-bundle') {
    ...
    if (directory !== '.sankiwork' || extra !== undefined || !digestIsHex) process.exit(8);
  ...
  await writeFile(path.join(profileRoot, 'package.json'), '{}\\n', 'utf8');
  if (process.env.SW_DSH_SETUP_FAKE_MODE !== 'probe-fail') await writeFile(marker, 'ok', 'utf8');
} else {
  process.exitCode = 2;
}
`, 'utf8');
  ...
  delete process.env.SW_DSH_SETUP_FAKE_MODE;
  return {
    options: { projectRoot: root, resourceRoot, runtimeDataDir },
    profileRoot: path.join(dshHome, 'profiles', 'sankiwork'),
    stateFile,
  };
}
...
describe('DeepSeek Harness companion setup', () => {
  ...
  it('quick-fails when the official dsh executable is absent', async () => {
    ...
    process.env.SW_AGENT_HOME = path.join(path.dirname(test.stateFile), 'empty-agent-home');
    process.env.PATH = '';
    await expectSetupError(
      installDeepSeekHarnessCompanion(test.options),
      'AGENT_NOT_INSTALLED',
  ...
  it('classifies install and post-install probe failures', async () => {
    ...
    process.env.SW_DSH_SETUP_FAKE_MODE = 'install-fail';
    await expectSetupError(
    ...
    process.env.SW_DSH_SETUP_FAKE_MODE = 'probe-fail';
    await expectSetupError(
  ...
  it('keeps the bundled package path out of the dsh Windows shell boundary', async () => {
    const test = await fixture();
    process.env.SW_DSH_SETUP_FAKE_MODE = 'require-profile-bundle';
