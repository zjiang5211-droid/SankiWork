...
describe('browser use diagnostics', () => {
  it('detects Browser Use prompts from the SankiWork Browser menu', () => {
    expect(isBrowserUseRequested('hello')).toBe(false);
    expect(isBrowserUseRequested('@agent-browser\n\nBrowser tab context:')).toBe(true);
    expect(isBrowserUseRequested('Use the selected SankiWork Browser tab as the bound target.')).toBe(true);
  });
