...
export function registerMcpRoutes(app: Express, ctx: RegisterMcpRoutesDeps) {
  ...
  const { SW_BIN, RUNTIME_DATA_DIR, PROJECTS_DIR } = ctx.paths;
  ...
  // even when `sw` isn't on the user's PATH (the common case for source clones
  ...
  function computeInstallPayload(): McpInstallPayload {
    const cliPath = SW_BIN;
    // The daemon was bootstrapped as a sidecar (tools-dev, packaged) iff
    // bootstrapSidecarRuntime stamped SW_SIDECAR_IPC_PATH into the env.
    // In sidecar mode the snippet omits --daemon-url and the spawned
    // `sw mcp` discovers the live URL via the concrete IPC endpoint on
    // every spawn, so the client config survives ephemeral-port
    // restarts. For direct `sw` / `sw --port X` launches there is no
    ...
    const mcpBootstrapCommand = process.env.SW_MCP_BOOTSTRAP_COMMAND;
    if (
      mcpBootstrapCommand != null
      && mcpBootstrapCommand.length > 0
    ) {
      sidecarEnv.SW_MCP_BOOTSTRAP_COMMAND = mcpBootstrapCommand;
    }
    const mcpBootstrapArgs = process.env.SW_MCP_BOOTSTRAP_ARGS;
    if (mcpBootstrapArgs != null && mcpBootstrapArgs.length > 0) {
      sidecarEnv.SW_MCP_BOOTSTRAP_ARGS = mcpBootstrapArgs;
    }
    // tools-dev / packaged launchers export SW_WEB_PORT so the daemon
    // knows where the browser-facing SankiWork studio is running.
  ...
  const CODEX_MCP_NAME = 'sankiwork';
  ...
  // External MCP server configuration. SankiWork connects to these as a
  ...
  // where SW_PUBLIC_BASE_URL pins the externally-routable URL.
  // ─────────────────────────────────────────────────────────────────
...
function renderOAuthResultPage(opts: any) {
  ...
  const body = ok
    ? `Your MCP server <code>${escapeHtml(opts.serverId ?? '')}</code> is now connected. You can close this tab and return to SankiWork.`
...
<head>
<meta charset="utf-8" />
<title>${escapeHtml(title)} — SankiWork</title>
  ...
  <script>
    try {
      ...
      if (window.BroadcastChannel) {
        var bc = new BroadcastChannel('sankiwork-mcp-oauth');
