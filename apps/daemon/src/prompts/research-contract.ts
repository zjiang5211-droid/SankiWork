...
export function renderResearchCommandContract(
  ...
  const lines = [
    ...
    '',
    '```bash',
    `"$SW_NODE_BIN" "$SW_BIN" research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```powershell',
    `& $env:SW_NODE_BIN $env:SW_BIN research search --query "<search query>" --max-sources ${maxSources}`,
    '```',
    '',
    '```cmd',
    `"%SW_NODE_BIN%" "%SW_BIN%" research search --query "<search query>" --max-sources ${maxSources}`,
