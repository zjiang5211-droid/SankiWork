...
// disabled when `SW_SNAPSHOT_GC_INTERVAL_MS` is set to `0`.
//
// Operator escape hatch: `sw plugin snapshots prune --before <ts>` calls
...
export function startSnapshotGc(opts: SnapshotGcOptions): SnapshotGcHandle {
  ...
  const tick = () => {
    try {
      // Plan §3.M1 / spec PB2 — feed SW_SNAPSHOT_RETENTION_DAYS into
