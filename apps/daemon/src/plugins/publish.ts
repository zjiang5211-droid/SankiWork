// Phase 4 / spec §14.1 — `sw plugin publish --to <catalog>` PR-template launcher.
//
...
//   - sankiwork            → nexu-io/open-design (plugins/community/<plugin-name>/).
  ...
  switch (args.catalog) {
    ...
    case 'sankiwork': {
      const bodyWithRegistry = [
        body,
        '',
        '## SankiWork registry entry',
        '',
        '- Target path: `plugins/community/<plugin-name>/sankiwork.json`',
        '- Generated index: `plugins/registry/community/sankiwork-marketplace.json`',
        '- Required checks: `sw plugin validate`, `sw plugin pack`, integrity digest, preview smoke.',
      ].join('\n');
...
export function upsertMarketplaceJsonEntry(args: {
  ...
  const manifest: MarketplaceJsonManifest = {
    ...
    name: typeof existing.name === 'string' ? existing.name : 'sankiwork-marketplace',
...
function renderPrBody(m: PublishMetadata): string {
  ...
  lines.push('- Ships `sankiwork.json` sidecar (additive SankiWork metadata).');
  lines.push('');
  lines.push('Generated by `sw plugin publish` — see https://sanki-ai.cloud/docs/plugins-spec.md.');
  return lines.join('\n');
}
