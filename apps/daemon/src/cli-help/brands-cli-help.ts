// Help surface for `sw brand`. Kept pure and separate from cli.ts so a test can
...
export const BRAND_USAGE = `Usage:
  sw brand list [--json]               List extracted brands (id, name, domain, status).
  sw brand extract <url> [--json]      Start an agent-driven brand extraction. Reserves the
  sw brand create <url> [--json]       brand and a backing project with the site open in a
  ...
  sw brand continue <id> [--json]      Restart the deterministic extraction pass for an
                                       existing brand/project/design-system without creating
                                       a duplicate design system.
  sw brand preview <id> [--json]       Re-render brand.html from the project's current
  ...
  sw brand finalize <id> [--json]      Register the agent's extracted kit (brand.json in the
                                       backing project) as a design system; marks it ready.
                                       --project <projectId> overrides the backing project.
                                       --locale <locale> overrides the stored brand locale.
  sw brand extract-from-html <id> --html-file <path|->
  ...
  sw brand get <id> [--json]           Print one brand's full detail (meta + brand + guide).
  sw brand delete <id> [--json]        Remove a brand and its registered design system.
