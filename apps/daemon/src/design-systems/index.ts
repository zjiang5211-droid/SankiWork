...
export function isDesignTokenChannelEnabled(
  ...
  return env.SW_DESIGN_TOKEN_CHANNEL !== '0';
}
...
async function designSystemAssetsCacheFingerprint(
  ...
  const payload = {
    tokenChannel: env.SW_DESIGN_TOKEN_CHANNEL ?? null,
...
// and attributes it to the SankiWork open-source project for shareability.
export function buildDesignSystemSkillsMarkdown(input: {
  ...
  lines.push('');
  lines.push(
    'Generated with **SankiWork** — the open-source, local-first Claude Design alternative. ' +
...
async function migrateLegacyDesignSystemPackage(
  ...
  const title = normalizeTitle(metadata.title ?? firstHeading(body) ?? id);
  const summary = summarize(body) || 'A reusable SankiWork design system.';
  ...
  await Promise.all([
    ...
    appKitExists
      ? writeIfMissing(
          ...
          `# ${title} UI Kit\n\nThis package was migrated from an earlier SankiWork design-system workspace. Use \`index.html\` as the applied interface example and replace it with source-backed modular components when new repository evidence is available.\n`,
        )
...
function generatedDesignSystemFileWrites(
  ...
  const summary = input.summary || 'A user-created SankiWork design system.';
...
function buildDraftDesignSystemBody(input: UserDesignSystemInput & { title: string }): string {
  ...
  const summary = cleanText(input.summary) || 'A user-authored design system for future SankiWork projects.';
...
A reusable SankiWork package for ${input.title}.
...
function renderSkill(input: {
  ...
  return `---
name: ${skillName}
description: Use this skill when generating SankiWork artifacts that should follow ${input.title}.
user-invocable: true
---
...
function renderOverviewHtml(
  ...
  return renderHtmlDocument(
    title,
    `<main class="overview">
      <p class="eyebrow">SankiWork system</p>
