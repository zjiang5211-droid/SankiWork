...
description: |
  ...
  SankiWork browser-tab context, testing web apps, dogfooding SankiWork
  previews, QA, bug hunts, or reviewing app quality. Prefer local SankiWork
...
Use `agent-browser` for local SankiWork preview validation: inspect rendered
...
For selected SankiWork browser tabs and browser-use/browser-harness-style
...
5. If the selected SankiWork context only provided a URL/title and no browser
...
Do not run SankiWork's own daemon CLI as a browser automation tool. Commands
such as `od browser snapshot`, `daemon-cli.mjs browser snapshot`, or
`$SW_NODE_BIN $SW_BIN browser snapshot` are not valid browser tools; they can be
...
With the SankiWork preview at `http://127.0.0.1:17573/`, run:
