...
inputs:
  ...
  - id: imagery
    ...
    schema_path: ../sankiwork-landing/schema.ts#ImageryConfig
parameters:
...
example_prompt: |
  ...
  card. Reuse the sankiwork-landing image library.
---

# sankiwork-landing-deck

Sister skill to [`sankiwork-landing`](../sankiwork-landing/). Same
...
```text
inputs.json + ../sankiwork-landing/styles.css
        │
        └──────────► scripts/compose.ts
                            │
                            ▼
...
`sankiwork-landing` brief, copy `brand` and `imagery` over verbatim.
...
script — it shares the 16-slot library from `sankiwork-landing`. To
regenerate or stub:

```bash
# generate via gpt-image-2 (fal.ai)
FAL_KEY=... npx tsx ../sankiwork-landing/scripts/imagegen.ts ../sankiwork-landing/inputs.example.json --out=../sankiwork-landing/assets/

# or paper-textured SVG placeholders
npx tsx ../sankiwork-landing/scripts/placeholder.ts ../sankiwork-landing/assets/
```

Set your deck's `inputs.imagery.assets_path` to wherever those PNGs
live (default in the example: `../sankiwork-landing/assets/`).
...
stylesheet from `../sankiwork-landing/styles.css`, layers
...
- **Reuse the sister skill's stylesheet.** The composer reads
  `../sankiwork-landing/styles.css` at compile time. Do not
...
- [`sankiwork-landing`](../sankiwork-landing/) — landing page sister skill.
