...
Run/project options:
  --daemon <url>               Daemon base URL. Falls back to SW_DAEMON_URL,
                               SW_PORT, then tools-dev status discovery.
...
async function resolveDaemonUrl(config: BatchConfig): Promise<string> {
  ...
  if (isDiscoverablePort(process.env.SW_PORT)) return `http://127.0.0.1:${process.env.SW_PORT}`;
...
async function main(): Promise<void> {
  ...
  if (resolved.dryRun) {
    ...
    const daemonUrl =
      config.daemonUrl ??
      process.env.SW_DAEMON_URL ??
      (isDiscoverablePort(process.env.SW_PORT) ? `http://127.0.0.1:${process.env.SW_PORT}` : '(not resolved in dry-run)');
