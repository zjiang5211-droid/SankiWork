---
name: sw-plugin-contribute-sankiwork
description: Open a pull request adding a local SankiWork plugin to the SankiWork community catalog using gh CLI.
triggers:
  - contribute plugin
  - open design pr
  - github pull request
...
Use this workflow when the active project contains a copied plugin folder and the user wants to propose it for the SankiWork community catalog.
...
3. Call the local daemon endpoint instead of hand-rolling GitHub commands:
   `curl -sS -X POST "$SW_DAEMON_URL/api/projects/$SW_PROJECT_ID/plugins/contribute-sankiwork" -H 'content-type: application/json' -d '{"path":"<plugin_context_path>"}'`
...
Keep the pull request focused. Do not modify unrelated SankiWork files unless a manifest validation issue requires a tiny supporting change.
