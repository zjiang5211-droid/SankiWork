...
- Keep runnable plugin examples portable: every example should have a `SKILL.md`; add `sankiwork.json` only as the OD sidecar.
- Keep `SKILL.md` bodies free of OD-only marketplace metadata. Put OD display, inputs, preview, pipeline, capabilities, and source information in `sankiwork.json`.
...
- Use the v1 JSON schema at `docs/schemas/sankiwork.plugin.v1.json`.
