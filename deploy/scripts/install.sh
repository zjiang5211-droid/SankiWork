...
printf "${BOLD}  │${RESET}   ${CYAN}◈${RESET}  ${BOLD}SankiWork${RESET}                     ${BOLD}│${RESET}\n"
...
# ---------------------------------------------------------------------------
# 2b. Check if SankiWork is already running
# ---------------------------------------------------------------------------
if $CONTAINER_CMD ps --filter "name=sankiwork" --format '{{.Names}}' 2>/dev/null | grep -q 'sankiwork'; then
  STATUS="$($CONTAINER_CMD inspect --format '{{.State.Status}}' sankiwork 2>/dev/null || echo 'unknown')"
  IMAGE="$($CONTAINER_CMD inspect --format '{{.Config.Image}}' sankiwork 2>/dev/null || echo 'unknown')"
  PORTS="$($CONTAINER_CMD port sankiwork 2>/dev/null || echo 'unknown')"

  error "SankiWork is already running."
  printf "\n"
  printf "  Container:  sankiwork\n"
  printf "  Status:     %s\n" "$STATUS"
  printf "  Image:      %s\n" "$IMAGE"
  printf "  Ports:      %s\n" "$PORTS"
  printf "\n"
...
SANKIWORK_IMAGE=${IMAGE}
SANKIWORK_PORT=${PORT}
SANKIWORK_ALLOWED_ORIGINS=${ALLOWED_ORIGINS}
SANKIWORK_MEM_LIMIT=${MEM_LIMIT}
NODE_OPTIONS=--max-old-space-size=192
SW_API_TOKEN=${GENERATED_TOKEN}
ENVFILE
...
if [ "$OS" = "Linux" ] && [ "$OPT_NO_SYSTEMD" = "0" ]; then
  if command -v systemctl >/dev/null 2>&1; then
    ...
    SYSTEMD_UNIT="${SYSTEMD_DIR}/sankiwork.service"
    ...
    {
      echo "[Unit]"
      echo "Description=SankiWork daemon (${CONTAINER_RUNTIME} Compose)"
    ...
    systemctl --user enable sankiwork 2>/dev/null || true
    ok "systemd unit installed: sankiwork.service"
    step "Manage with: systemctl --user start|stop|status sankiwork"
  else
...
printf "  Image:        %s\n" "$IMAGE"
printf "  Data volume:  sankiwork_data\n"
printf "  Config:       %s\n" "$ENV_FILE"
if [ "$OS" = "Linux" ] && [ -f "${HOME}/.config/systemd/user/sankiwork.service" ]; then
  printf "  Service:      systemd (sankiwork.service)\n"
fi
printf "\n"
printf "  Next steps:\n"
