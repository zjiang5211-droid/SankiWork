...
This deployment ships SankiWork as a single Alpine-based runtime image. The
...
2. Generate a secure token (recommended unless your reverse proxy will both authenticate every request and set `SANKIWORK_DISABLE_API_AUTH=1`):
...
3. Open `.env` in your editor and choose one auth mode:
   - default: paste the token into `SW_API_TOKEN=`
   - trusted reverse proxy that already authenticates every request: leave `SW_API_TOKEN=` empty and set `SANKIWORK_DISABLE_API_AUTH=1`

Then pull and start the service:

```bash
SANKIWORK_IMAGE=ghcr.io/nexu-io/od:latest docker compose pull
SANKIWORK_IMAGE=ghcr.io/nexu-io/od:latest docker compose up -d --no-build
```
...
`sankiwork` as the username and the `SW_API_TOKEN` value from `.env` as the
...
`Authorization: Bearer <SW_API_TOKEN>`.
...
must open GitHub -> Packages -> `sw` -> Package settings and change visibility
...
- Host port: `127.0.0.1:7456` (`SANKIWORK_PORT=8080` to publish on `127.0.0.1:8080`)
...
- Compose memory cap: `384m` (`SANKIWORK_MEM_LIMIT=256m` to override)
...
proxy, set `SANKIWORK_ALLOWED_ORIGINS` to the exact browser origins that should
be allowed to call `/api`:

```bash
SANKIWORK_ALLOWED_ORIGINS=https://od.example.com,http://203.0.113.10:7456 docker compose up -d --no-build
```

If the reverse proxy already authenticates every request and you do not want it
to inject `Authorization: Bearer <SW_API_TOKEN>` upstream, set:

```bash
SANKIWORK_DISABLE_API_AUTH=1
```
...
Compose variable maps to daemon env `SW_DISABLE_API_AUTH`.

Pin a specific published image with a digest instead of the mutable `latest` tag:

```bash
SANKIWORK_IMAGE=ghcr.io/nexu-io/od@sha256:<digest> docker compose up -d --no-build
```
...
```bash
docker build -t sankiwork-local -f deploy/Dockerfile.local .
```

**2. Point `.env` at the local image:**

```bash
SANKIWORK_IMAGE=sankiwork-local
```
...
```bash
docker run --rm -v sankiwork_sankiwork_data:/data alpine chown -R 1001:1001 /data
```
...
```bash
SW_CODEX_SANDBOX=danger-full-access docker compose up -d --no-build
```
...
and complete the browser's native sign-in prompt with username `sankiwork` and
the `SW_API_TOKEN` value from `.env`. Host networking is no longer required for
