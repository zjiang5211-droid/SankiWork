...
- R3. The task should create a real plugin folder containing at minimum `SKILL.md` and `sankiwork.json`, with optional examples/assets when the user asks for them.
...
- R6. Home's intent rail exposes a "Create plugin" chip that reuses the same authoring handoff, prompt contract, and `sw-plugin-authoring` scenario as the Plugins entry.
...
- No new plugin spec dialect. The generated folder must validate against the existing SankiWork plugin shape in `docs/plugins-spec.md` and `docs/schemas/sankiwork.plugin.v1.json`.
...
- Use the shipped dedicated bundled scenario `sw-plugin-authoring`. The `sw-new-generation` fallback remains only a compatibility bridge when an older registry does not contain the authoring scenario.
...
- [x] U3. **Plugin authoring scenario**
  ...
  **Files:**
  - Create: `plugins/_official/scenarios/sw-plugin-authoring/sankiwork.json`
  - Create: `plugins/_official/scenarios/sw-plugin-authoring/SKILL.md`
  ...
  **Approach:**
  - The scenario should instruct the agent to output a folder such as `generated-plugin/` with `SKILL.md`, `sankiwork.json`, and optional `examples/` or `assets/`.
  ...
  **Test scenarios:**
  ...
  - Regression: regular `sw-new-generation` and migration scenarios are unaffected.
...
- [x] U4. **Generated folder visibility in project output**
  ...
  **Approach:**
  - Preserve relative paths so `generated-plugin/sankiwork.json` and `generated-plugin/SKILL.md` remain visibly grouped.
  ...
  **Test scenarios:**
  - Happy path: a project with `generated-plugin/sankiwork.json` and `generated-plugin/SKILL.md` renders a folder group.
...
- sankiwork.json with valid metadata, mode, task kind, inputs, and any pipeline/context references
