...
SankiWork should show the signed-in AMR wallet balance inside the local
...
truth in Vela/AMR and does not add a new SankiWork billing model.

## Problem Statement

The SankiWork client can show that the user is signed in to AMR, but it does
...
- Link owns the Redis projection used for model-call preflight and pending
  usage.
- SankiWork daemon owns AMR local auth, CLI config, run preflight, and the
  web/CLI surfaces.
...
- Show AMR wallet balance in SankiWork after the user signs in to AMR.
- Keep SankiWork balance display sourced from Vela API, not from Link Redis.
- Avoid adding an API-to-Link read path for wallet display.
- Avoid adding Redis to the Vela API service for this feature.
- Avoid excess DB reads from the SankiWork client by adding daemon-side short
...
- Expose the same account snapshot to web and `sw` CLI surfaces.
...
- Do not show usage history, recharge history, billing summary, or checkout
  controls inside SankiWork.
- Do not read Link Redis from Vela API or SankiWork.
...
- Do not promise that the displayed balance already subtracts in-flight pending
  usage.
- Do not add a new SankiWork billing database, wallet store, or persisted
  balance cache.
...
SankiWork displays the AMR ledger wallet balance returned by Vela API.
...
insufficient balance, SankiWork should show the existing insufficient-balance
...
SankiWork currently exposes AMR login status through:
...
SankiWork's AMR config shape already recognizes profile fields including
...
balance display from SankiWork daemon requires the wallet balance route to
...
SankiWork balance display must not read those Redis keys.
...
Extend `GET /api/v1/wallet/balance` so SankiWork daemon can call it with the
...
- Return the existing response shape unless Vela wants to add optional
  metadata. SankiWork v1 can work with `balanceUsd` and `updatedAt`.

This is an external dependency for the SankiWork workstream, not part of the
SankiWork implementation slice. SankiWork must still handle the current
state where this endpoint returns unauthorized for daemon-held credentials.

## SankiWork Daemon API
...
Add or extend an `sw` command that exposes the same daemon snapshot, for
example:
...
The main DB-load mitigation lives in SankiWork daemon:
...
- Redis outages do not directly affect SankiWork wallet display.
- Redis outages may still affect Link run admission or model calls.
- SankiWork should keep showing the wallet ledger balance if Vela API is
  available, but should not infer that a run can start.

If Link rejects a run because projection is unavailable, SankiWork should use
...
### Slice 1: SankiWork Contracts and Daemon Client
...
- Add or extend `sw` AMR/status command to call the daemon wallet endpoint.
- Support `--json`.
...
Do not require Vela repository validation for the SankiWork-only workstream.
If the external Vela dependency is not deployed yet, SankiWork validation
