...
Define the target observability loop that lets SankiWork improve agent
...
SankiWork already reports completed runs to Langfuse and PostHog. The current
...
That means SankiWork can inspect individual runs, but the system does not yet
...
The gap is therefore not "does SankiWork know how to talk to object storage?"
...
SankiWork should treat observability as a closed loop:
...
alerting surface. SankiWork owns the domain model that maps a design-agent run
...
SankiWork is a design product, so the input and output surface is heavier than
...
| --- | --- |
| SankiWork object storage | Stores original user attachments, parsed text, thumbnails, OCR or embedding outputs, generated artifacts, preview screenshots, export bundles, and reproducibility files. |
...
- always emit an `attachment_manifest` and `artifact_manifest` with stable ids,
  ...
  redaction status, and SankiWork storage references;
...
- store durable `sankiwork://`-style references, object ids, or artifact ids that Open
  ...
  "attachment_manifest": [
    {
      ...
      "storage_ref": "sankiwork://objects/workspaces/ws_1/projects/proj_1/runs/run_1/attachment/att_123"
    }
  ],
  "artifact_manifest": [
    {
      ...
      "redacted": false,
      "truncated": false,
      "storage_ref": "sankiwork://objects/workspaces/ws_1/projects/proj_1/runs/run_1/artifact/art_456",
      "thumbnail_ref": "sankiwork://objects/workspaces/ws_1/projects/proj_1/runs/run_1/preview/prev_thumb_789"
    }
  ]
}
```
...
SankiWork should add a registry layer above `ProjectStorage` instead of
...
```text
sankiwork://objects/workspaces/<workspaceId>/projects/<projectId>/runs/<runId>/<objectClass>/<objectId>
```
...
apply, SankiWork should not write a score value; it should record
...
- Define the SankiWork object storage contract for attachments, parsed
  derivatives, generated artifacts, preview screenshots, export bundles, and
  reproducibility files.
- Define the registry schema and `sankiwork://` reference format for `attachment`,
  `parsed_attachment`, `artifact`, `preview`, `export`, and `replay` objects.
- Store original heavy inputs and outputs in SankiWork controlled storage,
