...
- MCP live artifacts are currently generated by `buildLiveArtifactsMcpServersForAgent`, which creates the `sw mcp live-artifacts` server config when `def.mcpDiscovery === 'mature-acp'`. Source: `apps/daemon/src/agents.ts:1111-1121`
...
- The executable resolver has module-level `cachedToolchainDirs`/toolchain path behavior, with tests covering SW_AGENT_HOME, NPM_CONFIG_PREFIX, VP_HOME, PATHEXT, fallbackBins, and configured `*_BIN` overrides. Source: `apps/daemon/src/agents.ts:900-983`; `apps/daemon/tests/agents.test.ts:1060-1361,1858-2004`
...
- Verification commands should use daemon-scoped checks: `pnpm --filter @sankiwork/daemon typecheck` and `pnpm --filter @sankiwork/daemon test`. Source: `apps/AGENTS.md:39-46`; `specs/change/20260509-agents-ts-split/agents-merge-conflict-report.md:244-249,262-266`
...
- Executables: preserve coverage for configured `*_BIN`, PATH lookup, fallback binaries, toolchain dirs, SW_AGENT_HOME, NPM_CONFIG_PREFIX, VP_HOME, Windows PATHEXT, and missing executable cases. Source: `apps/daemon/src/agents.ts:900-983`; `apps/daemon/tests/agents.test.ts:1060-1361,1858-2004`
...
- Validation: run `pnpm --filter @sankiwork/daemon typecheck` and `pnpm --filter @sankiwork/daemon test` after each implementation step. Source: `apps/AGENTS.md:39-46`; `specs/change/20260509-agents-ts-split/agents-merge-conflict-report.md:244-266`
...
- [x] Step 3: Split tests by responsibility
  ...
  - [x] Substep 3.3 Verify: run `pnpm --filter @sankiwork/daemon test` and ensure split tests still import compatibility APIs through the facade where relevant.
- [x] Step 4: Stabilize edge cases and review boundaries
  ...
  - [x] Substep 4.2 Verify: run `pnpm --filter @sankiwork/daemon typecheck` and `pnpm --filter @sankiwork/daemon test`.
...
- `pnpm --filter @sankiwork/daemon exec vitest run -c vitest.config.ts tests/runtimes` ✅
- `pnpm --filter @sankiwork/daemon exec vitest run -c vitest.config.ts tests/chat-route.test.ts` ✅ after one full-suite flaky failure in `tests/chat-route.test.ts`.
- `pnpm --filter @sankiwork/daemon test` ⚠️ runtime split tests passed; full suite still fails in existing unrelated `tests/finalize-design.test.ts` assertions where resolved artifact names include long relative temp paths.
