...
export async function buildWinLauncherPayloadArchive(
  ...
  const writeOverlay = async (input: { includeExecutable: boolean }): Promise<void> => {
    ...
    if (input.includeExecutable) {
      await cp(join(builtApp.unpackedRoot, "SankiWork.exe"), join(overlayRoot, "payload", "SankiWork.exe"));
    }
    await writeFile(
      join(overlayRoot, "payload", "resources", "sankiwork-config.json"),
...
export async function validateWinLauncherPayloadArchive(input: {
  ...
  try {
    ...
    requirePayloadManifestValue(manifest.entry?.executable, "entry.executable", "payload/SankiWork.exe");

    await stat(join(extractRoot, archiveRelativePath("payload/SankiWork.exe")));
    await stat(join(extractRoot, archiveRelativePath("payload/resources/sankiwork-config.json")));
