...
export async function prepareResourceTree(
  ...
  const node = {
    ...
    build: async ({ entryRoot }: { entryRoot: string }): Promise<ResourceTreeCacheMetadata> => {
      const resourceRoot = join(entryRoot, "sankiwork");
      ...
      return { resourceName: "sankiwork" };
    },
  };
  const manifest = await cache.acquire({
    materialize: options.materialize ? [{ from: "sankiwork", to: paths.resourceRoot }] : [],
    node,
  });
  return {
    key,
    resourceRoot: options.materialize ? paths.resourceRoot : join(manifest.entryPath, "sankiwork"),
  };
}
