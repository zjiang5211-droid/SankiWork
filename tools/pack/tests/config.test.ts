...
const savedTelemetryRelayUrl = process.env.SANKIWORK_TELEMETRY_RELAY_URL;
const savedPosthogKey = process.env.POSTHOG_KEY;
const savedPosthogHost = process.env.POSTHOG_HOST;
const savedAmrProfile = process.env.SANKIWORK_AMR_PROFILE;
const savedVelaWebUrl = process.env.SW_VELA_WEB_URL;

afterEach(() => {
  if (savedVelaWebUrl == null) {
    delete process.env.SW_VELA_WEB_URL;
  } else {
    process.env.SW_VELA_WEB_URL = savedVelaWebUrl;
  }
  if (savedTelemetryRelayUrl == null) {
    delete process.env.SANKIWORK_TELEMETRY_RELAY_URL;
  } else {
    process.env.SANKIWORK_TELEMETRY_RELAY_URL = savedTelemetryRelayUrl;
  }
  ...
  if (savedAmrProfile == null) {
    ...
    process.env.SANKIWORK_AMR_PROFILE = savedAmrProfile;
  }
});

describe("resolveToolPackConfig AMR profile", () => {
  it("bakes SANKIWORK_AMR_PROFILE into packaged config when set at build time", () => {
    process.env.SANKIWORK_AMR_PROFILE = "feature-test";
  ...
  it("rejects unsupported AMR profiles before packaging", () => {
    process.env.SANKIWORK_AMR_PROFILE = "staging";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /SANKIWORK_AMR_PROFILE must be prod, test, feature-test, or local/,
    );
  });
});
...
describe("resolveToolPackConfig telemetry relay", () => {
  it("reads and normalizes SANKIWORK_TELEMETRY_RELAY_URL for packaged config", () => {
    ...
    expect(config.telemetryRelayUrl).toBe("https://telemetry.sanki-ai.cloud/api/langfuse");
  });

  it("rejects invalid telemetry relay URLs", () => {
    process.env.SANKIWORK_TELEMETRY_RELAY_URL = "not-a-url";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /SANKIWORK_TELEMETRY_RELAY_URL must be an absolute https URL/,
    );
  });

  it("rejects plaintext telemetry relay URLs for packaged config", () => {
    process.env.SANKIWORK_TELEMETRY_RELAY_URL = "http://telemetry.sanki-ai.cloud/api/langfuse";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /SANKIWORK_TELEMETRY_RELAY_URL must use https/,
    );
  });
});
...
// sankiwork-config.json -> the packaged daemon spawn env (SW_VELA_WEB_URL).
describe("resolveToolPackConfig vela web origin", () => {
  it("bakes SW_VELA_WEB_URL into packaged config when set at build time", () => {
    process.env.SW_VELA_WEB_URL = "https://vela.example.invalid";
  ...
  it("rejects a vela web origin that is not an absolute http(s) URL", () => {
    process.env.SW_VELA_WEB_URL = "vela.example.invalid";
    expect(() => resolveToolPackConfig("mac")).toThrow(
      /SW_VELA_WEB_URL must be an absolute URL/,
    );
  });
});
