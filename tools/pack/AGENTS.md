...
- Local packaging orchestration for packaged SankiWork artifacts.
...
- Consuming sidecar/process/path primitives from `@sankiwork/sidecar-proto`, `@sankiwork/sidecar`, and `@sankiwork/platform`.
...
- Do not hand-build `--od-stamp-*` args; use `createProcessStampArgs` with `SANKIWORK_SIDECAR_CONTRACT`.
...
- Public release artifacts must use channel-specific app identity: stable uses `SankiWork`, beta uses `SankiWork Beta`, prerelease uses `SankiWork Prerelease`, and preview uses `SankiWork Preview`. Local tools-pack installs may still use namespace-scoped install paths only as a developer multi-instance validation convention.
...
- `apps/desktop/src/main/runtime.ts` exposes updater IPC to the renderer through `sw:update:status|check|download|install|quit` and emits `sw:update:status-changed`. Keep installer launch separate from process shutdown; quit is an explicit post-installer action.
...
The runtime updater reads `https://releases.sanki-ai.cloud/<channel>/latest/metadata.json` unless `SW_UPDATE_METADATA_URL` overrides it. For package-launcher updates:
...
- `SW_UPDATE_CURRENT_VERSION` may override the packaged version for tests, but user-flow package validation should prefer building the package with the intended `--app-version`.
...
- Post-update "what's new" highlights are NOT carried in release `metadata.json`. The daemon's `/api/whats-new` fetches a single hand-curated document on a dedicated R2 bucket (`https://whatsnew.sanki-ai.cloud/whats-new.json`, overridable with `SW_WHATS_NEW_URL`); the web home surface shows a one-time card driven by that document's `id`, not the running version. Operators edit that one file after a release — there is no per-version publish tooling.
...
- Stable: `SankiWork`, namespace `default` or stable release namespace.
- Beta Windows: `SankiWork Beta`, namespace `release-beta-win`, uninstall key `SankiWork-release-beta-win`.
- Prerelease Windows: `SankiWork Prerelease`, namespace `release-prerelease-win`, uninstall key `SankiWork-release-prerelease-win`.
- Preview Windows: `SankiWork Preview`, namespace `release-preview-win`, uninstall key `SankiWork-release-preview-win`.
...
```bash
SW_UPDATE_ENABLED=1
SW_UPDATE_METADATA_URL=<fixture metadataUrl>
SW_UPDATE_CURRENT_VERSION=99.0.0-beta.0
SW_UPDATE_OPEN_DRY_RUN=1
SW_UPDATE_AUTO_CHECK=1
```
...
```bash
curl.exe --ssl-no-revoke -fsSL https://releases.sanki-ai.cloud/beta/latest/metadata.json
```
...
```text
C:\odtp-beta-release-fixed\out\win\namespaces\release-beta-win\builder\SankiWork-release-beta-win-setup.exe
```

4. Expected user flow:

- User installs `0.8.0-beta.5` through the NSIS UI.
- User launches `SankiWork Beta`.
...
- For the payload path, the app downloads `platforms.win.artifacts.payload`, verifies sha256, prepares the payload under `%APPDATA%\SankiWork\launcher\channels\beta\namespaces\release-beta-win\versions\<version>\payload`, and shows the web updater popup.
...
- If the updater falls back to the installer path, clicking `Open installer` opens the real downloaded beta installer. Installing it should overwrite the same `SankiWork-release-beta-win` registry key, not create a second beta key.

5. Registry and launcher sanity check after beta.6 update:

```powershell
Get-ItemProperty 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Uninstall\*' -ErrorAction SilentlyContinue |
  Where-Object { $_.DisplayName -like 'SankiWork*' } |
  Select-Object PSChildName,DisplayName,DisplayVersion,InstallLocation

Get-Content "$env:APPDATA\SankiWork\launcher\channels\beta\namespaces\release-beta-win\runtime.json"
```

For a clean beta channel result, expect one beta entry with `PSChildName` `SankiWork-release-beta-win` and the latest `DisplayVersion`.
...
```bash
pnpm --filter @sankiwork/desktop test -- tests/main/updater.test.ts tests/main/updater-host-boundary.test.ts tests/main/preload-host-boundary.test.ts
pnpm --filter @sankiwork/web test -- tests/components/UpdaterPopup.test.tsx tests/lib/updater.test.ts
pnpm --filter @sankiwork/tools-serve test
pnpm --filter @sankiwork/tools-pack test -- tests/win-identity.test.ts tests/win-app.test.ts tests/win-builder.test.ts
