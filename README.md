<h1 align="center">SankiWork: The open-source Claude Design alternative</h1>

> ⚡ **[SankiWork Cloud — the official model service.](https://sanki-ai.cloud/zh/pricing/)** One recharge to use both agent and image models inside SankiWork: GPT, Claude, and DeepSeek for agents; GPT Image 2.0, Seedream 5.0 Pro, and Nano Banana 2.0 for images.
>
> 🚀 **[DeepSeek V4 Flash and V4 Pro are now available.](https://sanki-ai.cloud/zh/pricing/)** Put top-tier intelligence to work across prototypes, decks, design systems, and everyday agent tasks. SankiWork members can use both models without limits for two weeks, directly inside the app.
>
> 🧩 **[DeepSeek Harness is now supported.](https://sanki-ai.cloud/zh/agents/deepseek-harness-design/)** Connect DeepSeek's official `dsh` agent harness to SankiWork as a native runtime, with structured thinking, tool calls, model discovery, cancellation, and session resume. Generated files stay in the SankiWork workflow for live preview and delivery.

<p align="center">
  <img src="https://repo-assets.sanki-ai.cloud/resources/images/hero.png" alt="SankiWork hero banner — the headline &quot;The open-source Claude Design alternative&quot; over a classical scene of columns and robed figures on a digital-code backdrop, with stat cards for design systems, plugins, coding agents, and media providers" width="100%" />
</p>

<p align="center">
  <a href="https://sanki-ai.cloud/?utm_source=github&utm_medium=referral&utm_content=readme_website">Website</a> ·
  <a href="https://sanki-ai.cloud/?utm_source=github&utm_medium=referral&utm_content=readme_download">Download</a> ·
...
A quick look at the core SankiWork workflow. Start from **Home** with a brief, explore reusable skills in **Plugins**, and turn brand references into a **Design System**. Then enter a project's **Studio** to create and refine prototypes, decks, mobile apps, images, documents, and HyperFrames in one place.
...
<tr>
<td valign="top">
<img src="docs/screenshots/product-tour/home.png" alt="SankiWork Home page with artifact types, brief composer, model picker, and examples" /><br/>
...
<img src="docs/screenshots/product-tour/plugins.png" alt="SankiWork Plugins page showing the official skills catalog" /><br/>
<sub><b>Plugins</b> — browse official skills by category, search the catalog, and launch a workflow with <code>Try it</code>.</sub>
</td>
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/design-system.png" alt="Shopify design system preview inside SankiWork Studio" /><br/>
...
<tr>
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-prototype.png" alt="Web prototype preview in SankiWork Studio" /><br/>
...
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-deck.png" alt="Multi-slide deck preview in SankiWork Studio" /><br/>
<sub><b>Deck</b> — create multi-slide presentations, review thumbnails and speaker notes, and export when ready.</sub>
</td>
</tr>
<tr>
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-mobile-app.png" alt="Mobile app artifact preview in SankiWork Studio" /><br/>
...
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-image.png" alt="Generated image preview in SankiWork Studio" /><br/>
...
<tr>
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-document.png" alt="Multi-page document preview in SankiWork Studio" /><br/>
...
<td width="50%" valign="top">
<img src="docs/screenshots/product-tour/studio-hyperframe.png" alt="HyperFrame motion graphic preview in SankiWork Studio" /><br/>
...
| [Claude Desktop](https://claude.ai/download) | ✅ Supported¹ | `sw mcp install claude-desktop` |
...
| [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) | ✅ Native runtime | `sw agent setup deepseek-harness` |
...
For DeepSeek Harness, install the official `dsh` CLI first, then select it in SankiWork or run `sw agent setup deepseek-harness` to install/repair OD's connection component. For MCP integrations: `sw mcp install <agent> --print` for a dry-run preview · `--uninstall` to remove · full list with `sw mcp install --help`.

¹ Automatic MCP configuration for Claude Desktop is currently supported on macOS and Windows only.

<p align="center">
  <img src="https://repo-assets.sanki-ai.cloud/resources/images/coding-agents.png" alt="The 26 coding-agent CLIs SankiWork supports — DeepSeek Harness · Claude Code · Codex · OpenCode · Hermes · Antigravity · Vela · Grok Build · Kimi · Cursor Agent · Qwen · Qoder · GitHub Copilot · Pi · Kiro · Kilo · Mistral Vibe · DeepSeek · Reasonix · Aider · Amp · CodeBuddy · Mimo · AtomCode · Devin · Trae" width="100%" />
</p>
...
SankiWork (OD) is the open-source alternative. Same loop, same artifact-first mental model, none of the lock-in:
...
The fastest way to use SankiWork. No Node, no pnpm, no clone.

- **macOS** (Apple Silicon · Intel x64) → [**sanki-ai.cloud**](https://sanki-ai.cloud/?utm_source=github&utm_medium=referral&utm_content=readme_download_macos) or [GitHub Releases](https://github.com/nexu-io/open-design/releases)
- **Windows** (x64) → [**sanki-ai.cloud**](https://sanki-ai.cloud/?utm_source=github&utm_medium=referral&utm_content=readme_download_windows) or [GitHub Releases](https://github.com/nexu-io/open-design/releases)
...
You can use SankiWork without ever opening the GUI — call it as a skill, plugin, or MCP server inside Claude Code, Codex, Cursor, Copilot, OpenClaw, Antigravity, Hermes, Kimi, and more.

If you installed the macOS desktop app via the DMG or Homebrew cask, your shell
may still resolve `sw` to Apple's built-in `/usr/bin/od` octal-dump utility. In
...
`install.sh` is a thin shell wrapper around `sw mcp install`; it exists so the
hosted URL returns shell instead of the landing-page HTML fallback and fails
fast if your shell resolves a non-SankiWork `sw` binary.

> **macOS / WSL2 users:** `/usr/bin/od` is a system octal-dump command and can
> shadow SankiWork's `sw` command. Desktop-app users should prefer the
...
In a filesystem-backed local CLI run, the agent composes the selected functional skill or design template with your `DESIGN.md`, writes the canonical project files, and SankiWork previews those files. A BYOK/plain-API run without filesystem tools instead returns one complete `<artifact>` block.
...
If the browser asks for credentials, use `sankiwork` as the username and the
`SW_API_TOKEN` value from `deploy/.env` as the password. This keeps Docker bridge
traffic authenticated without requiring host networking. API clients can keep
using `Authorization: Bearer <SW_API_TOKEN>`.

### 🚀 Deploy on Sealos

[![Deploy on Sealos](https://sealos.io/Deploy-on-Sealos.svg)](https://sealos.io/products/app-store/open-design/)

The Sealos App Store template runs the published SankiWork Docker image with persistent workspace storage and Basic Auth on the public proxy. For custom public or shared Docker deployments, follow the reverse-proxy and `SANKIWORK_ALLOWED_ORIGINS` guidance in [`deploy/README.md`](deploy/README.md#local-compose).
...
5. **SankiWork gets smarter as you use it.** Your screenshots, fonts, palettes, and confirmed artifacts accumulate as defaults for the next session. Less rework, less drift.

---

## Use SankiWork from your coding agent

SankiWork ships a **stdio MCP server** and per-agent **install scripts**. Any MCP-compatible agent in another repo can read files from your local SankiWork projects directly — tokens CSS, JSX components, entry HTML — as a structured API queryable by name. The agent always sees the live file, not a stale export.
...
**For an agent starting from scratch,** the installer places `~/.config/<agent>/sankiwork.json` (or the platform equivalent) plus a copy-paste MCP snippet. Cursor gets a one-click deeplink; Claude Code gets a `claude mcp add-json` one-liner; every other agent gets JSON in the schema its config expects. On macOS desktop installs, prefer that Settings snippet over typing bare `sw mcp install <agent>` in Terminal, because `/usr/bin/od` may win on PATH. Full per-agent flow → **Settings → MCP server** in the desktop app, or [`docs/agent-adapters.md`](docs/agent-adapters.md).

**Security model.** Read-only by default, the daemon binds to `127.0.0.1`, and SSRF is blocked at the proxy edge. LAN exposure requires an explicit `SW_BIND_HOST` plus `SW_ALLOWED_ORIGINS`. Connector credentials and live-artifact preview routes stay loopback-only regardless.

**Internally-hosted model endpoints.** To prevent SSRF, the daemon blocks provider base URLs that resolve to private/internal address ranges (RFC1918, link-local, CGNAT, and cloud-metadata IPs) by default, surfacing `Internal IPs blocked`. If you run an internally-hosted gateway (e.g. LiteLLM or Ollama on a VPN-only `10.x`/`192.168.x` address), opt that host out with `SW_ALLOWED_INTERNAL_HOSTS=<host1>,<host2>,...` — a comma- or whitespace-separated list of bare hostnames or IPs (`10.0.0.5`, `litellm.internal.corp`; a `host:port` or full URL is accepted and reduced to its hostname; IPv6 must be bracketed, e.g. `[fd00::1]`). The allowlist is strict opt-in (empty by default), exact-host (no subdomain/substring matching), and applies **only** to provider endpoints you configure (connection test, model discovery, BYOK chat). It deliberately does **not** relax the guard on download URLs returned inside upstream responses, which stay blocked. A malformed entry — or CIDR notation, which is not supported — is dropped with a warning rather than silently trusted, so a typo never quietly widens (or fails to widen) the guard. Allowlisting a hostname trusts whatever it resolves to (like `SW_ALLOWED_ORIGINS`); allowlist the resolved IP instead if you want the DNS-resolved address re-checked.
...
**277 official plugins plus 183 remixable reference examples** live in [`plugins/_official/`](plugins/_official/). Each entry is a portable plugin directory anchored by `sankiwork.json` plus the payload required by its type: for example `SKILL.md` for agent workflows, `template.json` for media templates, or `DESIGN.md` for design-system entries. Jump straight to a category:
...
- 🔁 **Migrate Figma / Pencil workflows** → React, Next.js, or Vue source. See [`sw-figma-migration`](plugins/_official/scenarios/sw-figma-migration/).
- 🛠️ **Refresh an existing codebase to a brand spec** — point a plugin at a `git` repo + `DESIGN.md`, get a PR. See [`sw-code-migration`](plugins/_official/scenarios/sw-code-migration/).
- 💾 **Persist custom workflows** — your team's reusable templates sit next to the shipped ones.

### Using plugins

Plugins are at full parity across the **web UI** and the **`sw` CLI** — same `/api/plugins` endpoints, pick whichever fits.
...
An SankiWork plugin requires `sankiwork.json` plus the payload required by its type. A workflow skill or scenario also includes `SKILL.md`; manifest-only template and design-system entries use their own payloads instead:
...
1. Drop the plugin folder into [`plugins/community/`](plugins/community/) (third-party plugins), or — to ship it bundled with SankiWork — into the matching tier of [`plugins/_official/`](plugins/_official/).
2. Pass validation: `sw plugin validate`, `pnpm guard`, `pnpm --filter @sankiwork/plugin-runtime typecheck`.
...
- [x] **0.9.0** — SankiWork Cloud (official model service built into the app: zero config, one-click sign-in)
...
- [ ] `npx sw init` to scaffold a project with `DESIGN.md`
...
SankiWork keeps moving because contributors — designers, engineers, prompt authors — keep showing up. Many of the most-used skills, design systems, and plugins were written by people outside the core team.
...
| A new **plugin** | Drop `sankiwork.json` + the type-specific payload under a category folder | [`plugins/community/`](plugins/community/) · spec in [`plugins/spec/SPEC.md`](plugins/spec/SPEC.md) · agent dev guide in [`plugins/spec/AGENT-DEVELOPMENT.md`](plugins/spec/AGENT-DEVELOPMENT.md) |
...
We're recruiting **SankiWork Fellows** around the world — Fellows shape the product alongside the core team, represent SankiWork officially in their region, and grow the community locally, backed by funded support ($1,000 / MR), free LLM credits, and a direct review track. Details → [`MAINTAINERS.md`](MAINTAINERS.md) and the announcement on [Discord](https://discord.gg/mHAjSMV6gz).
...
| [`heygen-com/hyperframes`](https://github.com/heygen-com/hyperframes) | The HTML→MP4 motion-graphics framework, integrated as the first-class `hyperframes-html` in SankiWork. |
