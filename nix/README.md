...
This flake exposes SankiWork as a reproducible package, a `nix run` entry
point, a dev shell, and Home Manager / NixOS modules. The architecture
mirrors the runtime: the **daemon** (`sw` CLI, Express API on `/api/*`)
...
| Output                                     | What it is                                                                             |
| ------------------------------------------ | -------------------------------------------------------------------------------------- |
| `packages.<system>.daemon`                 | The `@sankiwork/daemon` package — produces `bin/od`. Default output.                 |
...
| `homeManagerModules.{default,sankiwork}` | Home Manager module — primary individual-developer interface.                          |
| `nixosModules.{default,sankiwork}`       | NixOS module — secondary, for shared/server installs.                                  |
...
```nix
{
  inputs.sankiwork.url = "github:nexu-io/open-design";

  outputs = { self, home-manager, sankiwork, ... }: {
    homeConfigurations.you = home-manager.lib.homeManagerConfiguration {
      modules = [
        sankiwork.homeManagerModules.default
        {
          services.sankiwork = {
            enable = true;
...
- Linux: `systemd --user` units `sankiwork.service` and (optionally)
  `sankiwork-web.service`. `systemctl --user status sankiwork`.
- macOS: `launchd` agents `io.nexu.sankiwork` and (optionally)
  `io.nexu.sankiwork-web`. `launchctl print gui/$UID/io.nexu.sankiwork`.
...
This creates a system user, drops a tmpfiles rule for `/var/lib/sankiwork`,
...
SankiWork's frontend is a static SPA that issues relative `/api/*`,
...
| `webFrontend.enable = false` (default) | You're running nginx / Caddy / Apache / Traefik yourself. Point your server's document root at `${pkgs.sankiwork.web}` (or the `packages.<system>.web` output) and replicate the proxy contract in section (4). |
...
> `services.sankiwork.webFrontend.allowedOrigins = [ "<your-proxy-origin>" ]`
> (which feeds `SW_ALLOWED_ORIGINS`) or, for the loopback-only
> split-port case, set `extraEnv.SW_WEB_PORT = "<proxy-port>"`. See
...
```nix
services.sankiwork.webFrontend = {
  enable = true;
...
services.sankiwork.openFirewall = true;
```

Under the hood `allowedOrigins` is forwarded to the daemon as the
`SW_ALLOWED_ORIGINS` environment variable (comma-separated). If you
run the daemon outside the modules — for example, behind your own
nginx/caddy — set `SW_ALLOWED_ORIGINS` directly in the daemon's
environment with the same shape:

```
SW_ALLOWED_ORIGINS=http://host1:port,https://host1:port,http://host2:port
```
...
The web package is built with `SW_DAEMON_URL = ""` so the bundled JS
issues **relative** requests — `/api/*`, `/artifacts/*`, `/frames/*` —
instead of baking a daemon URL into the export. There is no runtime
config endpoint; the SPA does not read `SW_DAEMON_URL` from the
serving environment.
...
- Document root → `${pkgs.sankiwork.web}` (or
  `packages.<system>.web`).
...
The static-server's environment does not need any SankiWork env
vars — but **the daemon's environment usually does**, because its
same-origin gate is built from `SW_BIND_HOST:port` (loopback hosts
...
| Proxy at a loopback host but different port (e.g. `http://127.0.0.1:8080` while daemon is on `:7457`)                                       | Either `extraEnv.SW_WEB_PORT = "8080"` (whitelists `8080` on every loopback host) or `services.sankiwork.webFrontend.allowedOrigins`.           |
| Proxy on any non-loopback host (LAN IP, mDNS name, Tailscale name, public domain — `https://od.example.com`, `http://laptop.local:5174`, …) | `services.sankiwork.webFrontend.allowedOrigins = [ "<full origin>" ]`. List every scheme + host[:port] combo a browser might load the SPA from. |

`webFrontend.allowedOrigins` is forwarded to the daemon as
`SW_ALLOWED_ORIGINS`; if you run the daemon outside the modules,
export `SW_ALLOWED_ORIGINS` directly with the same shape (see
...
Either renders to a file like `/run/secrets/sankiwork.env`; pass that
path:

```nix
services.sankiwork.environmentFile = "/run/secrets/sankiwork.env";
```
