...
SankiWork 采用语义化版本（SemVer `主.次.修订`），初始版本 **1.0.0**（2026-08-18 从 Open Design v0.19.2 基线重品牌后确立）；P2 品牌资产与数据兼容完成后升至 **1.1.0**；P2.5 内部命名收敛（`od-*` → `sw-*`，保留格式契约 `od-card`/`data-od-id`/遥测协议）完成后升至 **1.2.0**；P3 云端阶段（运行时协议 `od:`/`x-od-*`/`od_*` → `sw` 全量重命名 + marketplace 源切换到品牌仓库）完成后升至 **1.3.0**（当前版本）。

**硬性规则**：
1. **每次阶段完成必须更新版本号**。任何阶段（P1 品牌替换、P2 品牌资产、P3 云端等）完成后，AI 代理必须先升版本再提交，禁止不升版本直接提交。
2. **主产品版本 = 所有对齐包版本**，一处改动必须全量同步（当前对齐集：根 `package.json`、`apps/*`、`e2e`、`packages/contracts|download|host|launcher-proto|platform|release|sidecar|sidecar-proto`、`tools/dev|pack|release`，当前全部 1.3.0）。验证命令：
   ```bash
