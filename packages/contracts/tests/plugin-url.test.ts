...
describe('pluginDetailSlug (single segment = last id segment)', () => {
  it('takes the slugified last segment, dropping any namespace', () => {
    expect(pluginDetailSlug('sankiwork/Hero Deck')).toBe('hero-deck');
...
describe('pluginShareUrl', () => {
  it('defaults to the public sanki-ai.cloud origin, single-segment path', () => {
    expect(SANKIWORK_SITE_ORIGIN).toBe('https://sanki-ai.cloud');
    expect(pluginShareUrl('sankiwork/live-dashboard')).toBe(
      'https://sanki-ai.cloud/plugins/live-dashboard/',
    );
  });
