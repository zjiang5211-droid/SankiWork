...
This PR wires SankiWork's beta mac arm64 packaging path to Vela's npm-owned CLI distribution contract.
...
SankiWork depends only on the Vela meta package:

```json
"@powerformer/vela-cli": "0.0.1-test"
```

SankiWork does not depend directly on platform packages such as `@powerformer/vela-cli-darwin-arm64`. Vela owns that platform matrix through optional dependencies and its resolver API.

The Vela CLI binary resolution order is:

1. `SANKIWORK_VELA_CLI_BIN`
...
1. Vela publishes `@powerformer/vela-cli` to npm before SankiWork packaging starts.
2. SankiWork pins that package in `tools/pack/package.json` and `pnpm-lock.yaml`.
...
4. `tools-pack` enters the `resource-tree` phase and resolves/copies the Vela binary into the SankiWork resource tree.
5. `electron-builder` embeds that resource tree through `extraResources`.
6. The packaged daemon receives `SW_RESOURCE_ROOT` at launch and resolves AMR to `SW_RESOURCE_ROOT/bin/vela` unless `VELA_BIN` explicitly overrides it.

## Implemented Behavior

`tools-pack` now supports `--require-vela-cli`. When this flag is absent, missing Vela packages, unsupported platforms, missing resolvers, or null resolver results are treated as "skip Vela bundling." When this flag is present, packaging fails with an actionable error that mentions both remediation paths: install/use `@powerformer/vela-cli` or set `SANKIWORK_VELA_CLI_BIN`.

Vela resource copying now lives in `tools/pack/src/vela-cli.ts`, so the generic resource-tree helper only owns static SankiWork resources. The Vela helper resolves the binary through the shared resolver path and copies it into:

```text
resources/sankiwork/bin/vela
```
...
SankiWork should install only `@powerformer/vela-cli`. The meta package pulls the macOS arm64 binary package as an optional dependency on supported machines.
...
```text
.tmp/release-beta-dry-run/out/mac/namespaces/release-beta/resources/sankiwork/bin/vela
```
...
- SankiWork depends only on `@powerformer/vela-cli`.
- `SANKIWORK_VELA_CLI_BIN` remains highest priority.
