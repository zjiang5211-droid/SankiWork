...
This note describes how an external service can use SankiWork as a creative
runtime while keeping media provider governance outside SankiWork.

SankiWork contributes project context, skills, design systems, previews,
...
Run-scoped `mediaExecution` controls only SankiWork-owned media generation.
It applies to:

- token-gated `/api/tools/media/generate`
- in-run `sw media generate` when `SW_TOOL_TOKEN` is present
- SankiWork's Codex image generation prompt override
- SankiWork's media generation prompt contract
...
SankiWork should not grow a generic provider router, provider account pool,
...
Use HTTP/SSE between the external service and SankiWork. Avoid shelling out to
`sw` from the external service unless the integration specifically needs the CLI
contract.

1. The external service authenticates the caller and decides provider policy.
2. The external service creates or selects an SankiWork project.
3. The external service starts a run with `mediaExecution` set to the desired
   SankiWork-owned media policy.
4. The run includes skills and MCP tools that describe the external media
   workflow.
5. SankiWork handles design/runtime work. External MCP media tools handle
...
For runs where SankiWork may use only a narrow part of its own media path,
use allowlists:
...
Use SankiWork project context, design-system guidance, and artifact previews
to plan the campaign. When media bytes are needed, use the configured external
media MCP tools. Do not call SankiWork-owned media generation unless the run
...
without making SankiWork own provider auth or budgets.
...
the external service, not to SankiWork core.

The MCP server should receive its own credentials from the external service or
from its deployment environment. SankiWork should only see the MCP tool
...
- The external service uploads the fulfilled asset through an SankiWork
...
SankiWork project files. Project artifacts should describe creative output,
...
still available for normal SankiWork media generation outside the in-run tool
path. The accepted v1 policy closes the cooperative in-run CLI path by routing
`sw media generate` through `/api/tools/media/generate` when `SW_TOOL_TOKEN` is
present.
...
should use `SW_TOOL_TOKEN` and `/api/tools/media/generate` instead of depending
...
- `request-only` run mode in SankiWork core
- SankiWork media request persistence for external provider work
- a generic HTTP executor provider inside SankiWork
- provider credentials or account pools in SankiWork
- SankiWork-owned provider budgets, retries, or global media limits
