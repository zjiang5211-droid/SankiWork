...
This document records how to read the new SankiWork Langfuse trace shape for
...
content should remain in SankiWork object storage and be referenced by
...
Raw sensitive content belongs in SankiWork object storage with access control,
...
Issue #3733 targets the first durable bridge between SankiWork-owned object
...
SankiWork-owned project storage today and to a ProjectStorage or
...
uses Langfuse-safe manifest fields and `sankiwork://objects/...` references rather than
...
| Storage reference | Emits stable `sankiwork://objects/...` references. | Treats `storage_ref` as an SankiWork registry reference, future-compatible with ProjectStorage/S3 backends. | No conflict; current refs should remain non-dereferenceable from Langfuse. |
...
1. Make the manifest builder resolve through a concrete SankiWork object
...
4. Preserve `storage_ref` as a stable, non-secret reference and require any
   future download or preview access to go through SankiWork-controlled
   authorization.
...
- `pnpm install`
- `pnpm --filter @sankiwork/daemon exec vitest run -c vitest.config.ts tests/langfuse-trace.test.ts tests/langfuse-bridge.test.ts tests/claude-stream-thinking.test.ts`
...
1. Store complete raw/redacted prompt payloads in SankiWork object storage and
