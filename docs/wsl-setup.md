...
install and run SankiWork from WSL as well so the agent CLI, `sw` command,
...
- Clone SankiWork inside WSL2.
- Install Node `~24` and the repo-pinned pnpm (`10.33.2`) inside WSL2.
- Put a WSL-native `sw` wrapper before `/usr/bin` on `PATH`.
- Start the daemon from WSL with `sw --no-open`.
...
ambiguous. A WSL-started daemon keeps the MCP clients and SankiWork in the
same environment.

## 1. Install from source in WSL

```bash
git clone https://github.com/nexu-io/open-design.git ~/tools/sankiwork
cd ~/tools/sankiwork
...
## 2. Fix the `sw` command collision

Linux already ships `/usr/bin/od` (octal dump). If that binary wins on `PATH`,
commands such as `sw mcp install claude` fail with file-not-found messages for
...
If `/usr/bin/od` appears before SankiWork, create a wrapper in `~/.local/bin`
...
coreutils binary instead of SankiWork, especially on machines with Windows
coreutils installed.
...
```bash
curl -sSf http://127.0.0.1:7456/api/health && echo "SankiWork daemon is reachable"
```
...
```bash
sw mcp install claude
sw mcp install opencode
sw mcp install codex
sw mcp install antigravity
sw mcp install copilot
```
...
If dependencies were installed under Node 22 and SankiWork later runs under
...
```bash
pnpm --filter @sankiwork/daemon exec node -e "require('better-sqlite3')"
```
...
SankiWork also normalizes this shape before daemon-launched Codex runs, but
manual cleanup may still be needed when Codex itself is invoked directly before
SankiWork gets a chance to patch the config.
